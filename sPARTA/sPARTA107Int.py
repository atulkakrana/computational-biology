...
miRNA_file = 'AsparagusStrigent_23_24_miRNA.fa' ## If not present in directory than miRcode (below) will be sued to fetch miRNAs from miRbase table
...
fileFrag = 'N'                          ## Fragment file is 'Y' else use files split earlier - Not implemented
##
indexStep = 'N'
...
revMapstep = 'Y'                        ## Reverse map to match co-ordinates with our genome
standardCleave = 'N'
...
def tarParse3(targComb):
    ...
    for i in fh_in:
        ...
        for x,y in zip(gappos[:-1],gapNuc):## In gap pos list which is made of alphabet splitting there is always am empty value at end because string has alphabet at last
            #print(x,y)
            if y == 'I': ## There was an insertion (bulge) in miRNA and gap in reference
        ...
        #print('Target seq after manipulation: %s' % (''.join(tarHash)))
        #print('MiRNA after after maipulations: %s' % (''.join(mirHash)))
        ...
        if '^' in misinfoBlock: 
            ...
            for x,y in zip(misposCorrect,misNuc):
                ...
                tarHash[posCount-1+gaps+bulge] = y
