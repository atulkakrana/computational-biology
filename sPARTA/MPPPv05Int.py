...
GenomeDB = 'BRACHY_MIPS1_genome' ##DB to get fastaOut file
PAREdb = 'BRACHY_priv_PARE_v4' ##Make sure that your Library is in the DB
...
libs =['661_chopped.txt'] ## Name of lib files
...
def writeDensityFile(densityDict, mode, allHits, baseCounts, baseCountsOffTagLen,
    ...
    if(mode == 1):
        ...
        print('median = %s\ntwentyPercentile = %s\nseventyFivePercentile = '\
            '%s\nninetyPercentile = %s' % (globalMedian, twentyPercentile, 
            seventyFivePercentile, ninetyPercentile))
        ...
        if(mode == 0):
            ...
            for location in sorted(densityDict[gene].keys()):
                ...
                if(hits <= twentyPercentile):
                    ...
                    densityDict[gene][location] = (hits, 2)
                    catTwoAbun.append(hits)
                elif(hits > globalMedian and hits >= seventyFivePercentile and
...
def main():
    ...
    print('TPmode: %s | TPscore: %s | Uniq filter: %s' % (TPmode,TPscore,bowtieFilterSwitch))
    fh_run.write('TPmode:%s | TPscore: %s | Uniq filter:%s' % (TPmode,TPscore,bowtieFilterSwitch))
    FragStart = time.time()
