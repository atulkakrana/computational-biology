...
def TagAbundanceFile(con):
    ...
    os.mkdir('./PARE')
    cur = con.cursor()
    cur.execute('select distinct(lib_id) from AT_sbsML_PARE.run_master')##@@@@@@@@@@@@@@@@@CHANGE PARE DB NAME HERE@@@@@@@@@@@@@
    libs = cur.fetchall()
    print ('These are the libraries %s' % libs)
...
#        print (lib[0])
        ...
        cur.execute('select tag, raw_value from AT_sbsML_PARE.run_master where lib_id = %s', (lib[0],))##@@@@@@@@@@@@@@@@@CHANGE PARE DB NAME HERE@@@@@@@@@@@@@
...
#    TF_out.close()    
    ...
    fh_output.write('miRNA name,target_gene,target_location,mirna_seq,tar_seq,targetfinder_score,p-value\n')
...
def UniqScoringInp(validated_comb):
    ...
    for i in csv_in1: ##miRNA is 5th column i.e. i[6]
