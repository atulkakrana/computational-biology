...
#### MPPP is internal version of sPARTA
#### Written and maintained by kakrana@udel.edu

#### PYTHON FUNCTIONS ##############################
import sys,os,re,time,timeit,csv,glob, string,subprocess,multiprocessing,shutil,operator,pyfasta,numpy,bisect,datetime
...
miRNA_file = 'Maize_aTAS3_siRNAs.fa' 	## If not present in directory than miRcode (below) will be sued to fetch miRNAs from miRbase table
...
genomeFeature = 0 						## 0 for Gene and 1 for Intergenic
getPARELib = 'Y' 						## 'N': PARE libraries are present on Disk  OR 'Y:' need to be fetched
libs =['1028','1029','1030','1031','128','755','820','826','3098','3099','3100'] ## Name of PARE libraries (if getPARELib = 'N')

#### STEPS ######################
generateFasta = 'Y' 					## If Local = Y - Use GFF file and splice out genes
fileFrag = 'Y' 							## Fragment file is 'Y' else use files split earlier - Not implemented
##
indexStep = 'Y'
...
################################
##### Advanced Options #########
TPscore = 'R' 							## R -Rebel and 'N' for normal target scoring
...
cat4Filter = 'N' 						## Filter out Category 4 - Developer option
##### BE VERY CAUTIOUS ##########
    ...
    if noiseFilter == 'Y':
        for target in validatedTargets:
            ...
            if((float(target[pValueIndex]) <= .25) and (float(target[
                ...
                if(cat4Filter == 'Y' and (int(target[categoryIndex]) == 4)):
                    pass
                else:
...
def main():
    ...
    if predStep == 'Y':
        ...
        for tagCountFilename in libs:
            ...
            print("Creating DeGe Index dictionary for lib %s" % library)
            deGeStart = time.time()
...
###v095 -> v099
## Fixed the co-ordinates issue for scaffolds that is duplicated intergenic ientries were bing generated and last gene on scaffold was n ot being included - Fixed on Scaffold- need testing on assembled genome
