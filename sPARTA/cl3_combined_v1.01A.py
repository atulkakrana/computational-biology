...
##Dont forget to remove TEST SWITCH in TagAbundanceFile() module which makes acript work on just first PARE library
...
dataserver = 'raichu.dbi.udel.edu' ##Server where PARE DB to use
...
##9. Change Cleaveland p-value cutoff here
pvalue =0.025
...
def ConnectToDB(server):
    print ('\nTrying to connect to %s' % (server))
    # Try to connect to the database
    try:
        con=sql.connect(host= server, user='kakrana', passwd='livetheday')
...
def TagAbundanceFile(con):
    ...
    os.mkdir('./PARE')
    cur = con.cursor()
    cur.execute('select distinct(lib_id) from AT_sbsML_PARE.run_master')
    libs = cur.fetchall()
#    print (libs)
...
#        print (lib[0])
        ...
        cur.execute('select tag, raw_value from AT_sbsML_PARE.run_master where lib_id = %s', (lib[0],))
...
#        fh_in = openopen('%s_PARE_tags.fa' % (lib[0]), 'r')
        pipe = subprocess.Popen(["/data2/homes/kakrana/tools/bowtie-0.12.7/bowtie", "-v", mismatch,"--best", "--strata", "-k", report_seq, "-f", "/data2/homes/kakrana/tools/bowtie-0.12.7/indexes/arab_tair10_cdna_index", dd_file, map_out])
...
#    os.cd('./dd_map')
    ...
    pval = str(pvalue)##For final cleveland command
