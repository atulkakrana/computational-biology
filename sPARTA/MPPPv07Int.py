...
GenomeDB = 'MT_MIPS3_5_genome' ##DB to get fastaOut file
PAREdb = 'LEGUME_pub_PARE' ##Make sure that your Library is in the DB
...
libs =['272_chopped.txt','808_chopped.txt','809_chopped.txt'] ## Name of lib files
...
def pValueCalculator(target, targetFinderList, proportion):
    ...
    n = sum(x.count(miRNAName) for x in targetFinderList if x[5] == score) ##AK: Modifed to improve p-val
...
def writeValidatedTargetsFile(header, validatedTargets, outputFile):
    ...
    f.write(header + ',cleavage position,PARE reads,10 nt window abundance,'\
        'PARE reads/window abundance,category,p-value\n')
...
#### Index required for TP bug fixed
