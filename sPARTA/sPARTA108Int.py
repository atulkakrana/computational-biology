...
## sPARTA-Int v1.07 - Reached End of support -- Atul
...
miRNA_file = 'miR.fa' ## If not present in directory than miRcode (below) will be sued to fetch miRNAs from miRbase table
...
libs =['3485','3512','3513','3514','3515','3516','3517','3518','3622','3623','3624','3625','3626','3627'] ## Name of PARE libraries - Mandatory

#### STEPS ######################
generateFasta = 'N'                     ## If Local = Y - Use GFF file and splice out genes
fileFrag = 'Y'                          ## Fragment file is 'Y' else use files split earlier - Local compatible step
##
indexStep = 'Y'
tarPred = 'Y'                           ## Target prediction required or not 'Y' and 'N' - Local compatible step
tarScore= 'Y'                           ## Target scroing - Local compatible step
##
...
tableUpdate = 'N'                       ## Update to specified table
...
noiseFilter = 'Y'                       ## Remove noise from results i.e. corr p-val >= 0.25, PARE ratio <= 0.25 AND Cat 4 validations
...
def tarFind3(frag):
    ...
    if retcode == 0: ### Index creation sucessful or index already exists
        ...
        if TPmode == 'H': ## Heurustic
            ...
            readGap = str("22,14")
            refGap= str("8,14") ## Bulge in miR
            ### Changed -D 5 to 6, changed -R 1 to 2 | Jan 13 -D 6 -> -D 3
            retcode2 = subprocess.call(["bowtie2","-a","--end-to-end","-D 3","-R 2","-N 1","-L 8","-i",intervalFunc,"--rdg",readGap,"--rfg",refGap,"--min-score",minScoreFunc,"--norc","--no-hd","--no-unal","-p",nthread2, "-x", index, "-f" ,"miRinput_RevComp.fa","-S", file_out])
            ...
            readGap = str("8,14") ## Bulge allowed in gap -Mar-11-15
            refGap= str("8,14") ## Bulge in miR
            
            #### Jan 13 -D 7 -> -D 4 | 
            retcode2 = subprocess.call(["bowtie2","-a","--end-to-end","-D 4","-R 2","-N 1","-L 6","-i",intervalFunc,"--rdg",readGap,"--rfg",refGap,"--min-score",minScoreFunc,"--norc","--no-hd","--no-unal","-p",nthread2, "-x", index,"-f", "miRinput_RevComp.fa","-S", file_out])
...
def tarParse3(targComb):
    ...
    for i in fh_in:
        ...
        posCount = 0
        #print("\nThis is gappos:",gappos,"And gapNuc:",gapNuc)
        
        ## At this point both mirHash and tarHash has same length and perfect complemnetrity as tarHash is essentially reverse complemneted miRNA used for matching
        for x,y in zip(gappos[:-1],gapNuc):## In gap pos list which is made of alphabet splitting there is always am empty value at end because string has alphabet at last
            ...
            elif y == 'D': ## There was a deletion in miRNA and so bulge in reference and gap in miRNA - In this case length of both miRNA and target will increase
                for i in range(int(x)): ## For as many as gaps in miRNA                
                    mirHash.insert(posCount,'-') ## When counted in python insertion will occur after posCount value -  Correct
                    tarHash.insert(posCount,'^') ## Add bulge markers in target sequence as well
                    posCount += int(x)
        ...
        ## Check for '^'  means there was a deletion(gap) in miRNA -which has been added but replace the bulge '^' in target with actual sequence
        if '^' in misinfoBlock: 
            ...
            # print('This is mismatch info block:',misinfo)       
            ...
            # print('This is mispos:',mispos,'and misNuc',misNuc)
            
            ## After adding gaps now replace the nucleotides in tarHash to give actual targets
            ...
            for x,y in zip(misposCorrect,misNuc):
                # print(x,y)
                posCount += x ## Convert bowtie position to python format
                ## Account for gap and bulges before replacing the nucleotide with that in target
                ## Can give problem if more than one gap? But more than one gap not allowed V07 modification?
                gaps = tarHash[:posCount-1].count('-') ## Convert bowtie pos. to python format -  
                ## This is for bulge that are added in targets - Need to replace with real nucleotide
                bulge = tarHash[:posCount-1].count('^') ## Convert bowtie pos. to python format - This needs revisit- Does mismatchinfo accounts for bulges unlike gaps?
                # print("Gap:%s | Bulge:%s" % (gaps,bulge))
                tarHash[posCount-1+gaps+bulge] = y ## Replaced the bulge with nucleotide in target - OK
            
        else: ## Normal - Old without a gap in miRNA - OK
            ...
            ## After adding gaps now replace the numcletides in tarHash to give actual targets and also add nucleotide to target (replace ^) if gap in miRNA
            posCount = 0
            for x,y in zip(misposCorrect,misNuc):
                #print(x,y)
                posCount += x ## Convert bowtie position to python format
                ...
                ## Can give problem if more than one gap? But more than one gap not allowed V07 modification?
                gaps = tarHash[:posCount-1].count('-') ## Convert bowtie pos. to python format
                tarHash[posCount-1+gaps] = y ## OK- Tested
        ...
        # for x,y in zip(miRrevcomp[::-1].replace("T","U"),tar[::-1]): ## Orientation changed to read from 5' miRNA - Changed in v1.07 - Direct miRNAs sequence with '-' used for scoring
...
def TagAbundanceFile(con,PAREdb,libs):
        ...
        #print (lib[0])
            ...
            cur = con.cursor()
            cur.execute('SELECT tag, norm from %s.run_master WHERE lib_id = %s AND hits <= 20 ' % (PAREdb,alib))
...
def TableUpload(con3,uniqRevmapped,mir_tab_name,miRFile):###Con is connection and res_upload is file from last module which has genomic coords and need to be upload
    ...
    if TableWipe is 'Y': ## If the Global setting is 'Y' than table will be dropped - Be careful   
        ...
        cur.execute("TRUNCATE TABLE mir_master.mirPARE_pub_v3")## Clear table before writing
        #con2.commit()
    ...
    add_row = "INSERT INTO mir_master.mirPARE_pub_v3(mir_name,target_name,chr_id,strand,binding_site,cleavage_site,target_score,cat,mir_seq,target_seq,p_value,small_win,large_win,win_ratio,pval_corr,mismatch,CIGAR,PAREdb,miRdb_type) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def main():
    ...
    #uniqRevmapped = '/home/kakrana/1.PARE/1.Aspa/2.Inter/output/ALL_revamapped_Uniq.csv'
    if tableUpdate == 'Y':
