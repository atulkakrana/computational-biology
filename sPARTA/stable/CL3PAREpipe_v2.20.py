...
#trascriptome_file = 'A2_PART2_Clean_RevComp.fa'##The FASTA header will be cleaned automatically

##2. A miRNA File specified bu user - If file input used than table input will be turned off automatically
#miRNA_file = 'A2_miRNAs.fas'##The header will be cleaned automatically
#miRCode = 'ath'

##3. PARE database names
PAREdb = 'COTTON_sbsISU_PARE'

##3B. Genome database name
GenomeDB = 'MT_MIPS3_5_genome'
    ...
    try:
        ...
        for i in found:###In gene summarization record first two entries are gene_name and size of gene, last entry is a blank line when record is fetched (not in file)
            ...
            if locus[0].isdigit() and int(locus[0]) <= int(cleave_site+3): ###Fixes BUG 14-3-13, found one line in summarized file as ['#', 'Transcriptome=./genomic_seq.fa'] instead of like ['12101480', '2', '1', '0', '2'] AND Reads file till large window limit+ 1 extra
                ...
                #    pass
                #
                if int(locus[0]) <= int(cleave_site+5) and int(locus[0]) >= int(cleave_site-5):###Range of Large window###GET LARGE AND SMALL WINDOW IN SAME LOOP
...
###Aug 16
### Multiple mirBASE protection added
