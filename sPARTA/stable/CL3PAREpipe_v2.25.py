...
trascriptome_file = 'A2_pseudo_transcript.fas'##The FASTA header will be cleaned automatically

##2. A miRNA File specified bu user - If file input used than table input will be turned off automatically
miRNA_file = 'A2_miRNAs.fa2'##The header will be cleaned automatically
miRCode = 'goss'
...
##14. Make index or use existing (index must be in 'index' folder withih same directory)
make_index = 'Y'
...
def GetCoords(con,db):
    ...
    ###########################################################TEST LINE#################################################################
    #cur.execute("SELECT chr_id,strand,gene,start,end,type FROM %s.gene_master WHERE type NOT LIKE 'mirna' AND gene LIKE 'protein-coding' ORDER BY chr_id,strand,start" % (db))###extra % escapes the % in query###Limiting number of entries will effect last gene coords and sequence
    #####################################################################################################################################
...
def miRinput(con):
    ...
    #####################REAL WORLD - UNCOMMENT WHEN NOT-TESTING#############
        ...
        cur.execute("SELECT mir_name, accession, organism, mir_seq FROM mir_master.mirBASE WHERE mir_name like '%s%%'" % (miRCode))
...
####!!!!!!!!!!!!!!!!!!!!!!!!!!END OF TEST SECTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if LibPref == 0:
        ...
        for ent in lib_info:##All the entries of the library
            ...
            elif len(ent[0]) > tag_len:##For tag longer than specifed chop it to specified length
                chop_tag = ent[0][:tag_len]
                for count in range(ent[1]):##Number of times the tag_count file
                    fh_out.write('>%s\n%s\n' % (tag_num, chop_tag))
                    tag_num += 1
            else:
    ...
    #        print(lib_info[0])
            ...
            for ent in lib_info:##All the entries of the library
                ...
                elif len(ent[0]) > tag_len:##For tag longer than specifed chop it to cpecified length
                    chop_tag = ent[0][:tag_len]
                    for count in range(ent[1]):##Number of times the tag_count file
                        fh_out.write('>%s\n%s\n' % (tag_num, chop_tag))
                        tag_num += 1
...
def mapdd2trans(libs):##Creates index on fly and map PARE tags to index
    ...
    report_seq = str(20) ###number of sequences to report from bowtie mapping
    nproc2 = str(nproc)
    ...
    #        fh_in = openopen('%s_PARE_tags.fa' % (lib[0]), 'r')
            retcode2 = subprocess.call(["/data2/homes/kakrana/tools/bowtie-0.12.7/bowtie", "-v", mismatch,"--best", "--strata", "-k", report_seq, "-p",nproc2, "-f", "./index/an_index", dd_file, map_out])
...
def RevMapCoord(ent): ####DOES THIS MODULE WORKS IF USER USES MULTIPLE LIBRARIES - Parallelize
    ...
    if Local == 'N':
        ...
        #######################################UNCOMMENT IF REVERSE MAPPING OF CO-ORDINATES TO BE DONE####################################################
    ...
    #['>Aovel471', 'scaffold102681', '4523-4543', '4534', '3.5', '0.000303', 'UCCCUACUAAAGAUUUCGACA', 'UGGGAUGGUUUCUGAAGCUGA']
    if small_win != 0:
...
def TableUpload(con2,res_upload,mir_tab_name):###Con is connection and res_upload is file from last module which has genomic coords and need to be upload
    ...
    cur.execute("TRUNCATE TABLE kakrana_data.mirBASEPAREres_new")## Clear table before writing
    #con2.commit()
    ...
    add_row = "INSERT INTO kakrana_data.mirBASEPAREres_new (mir_name,target_name,chr_id,strand,binding_site,cleavage_site,target_score,mir_seq,target_seq,p_value,small_win,large_win,win_ratio,PAREdb,miRdb_type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
def main(): ###Variables need to be initialized in the ----main-------
    ...
    global nproc
    nproc = 1 ## mysql queries don't work on multiple cores
    ...
    if Local =='Y':
        ...
        for i in gene_coords:###gene_coords is a list in script, also written out as file of same name
            ...
            if strand == 'c':### if entry in reverse strand
                atuple = (i[0],i[4])
                coord_dict_crick[i[2]] = atuple###Gene name as key and chr_id,strand end as value
            else:
                atuple = (i[0],i[3])
                coord_dict_wat[i[2]] = atuple##Gene name as key and chr_id,strand end as value
    ...
    #TableUpload(con2,sco_inp_extend_geno,miRTable)
    ###############################################################################################################################
...
if __name__ == '__main__':
    ...
    ##GLOBAL VARIABLES###        
    ######TEST PARAMETERS######
    #genome_file = './A2_PART2_Clean_RevComp.fa'###Supplied by 'GetFASTA' function in REAL, Required by 'TargetFinder' as PP doesnt allow multiple arguments
    ...
    #all_libs = [(0,),(270,)] ## Supplied by 'TagAbundanceFile' function in REAL
    #only_libs = [(270,)] ## Supplied by 'TagAbundanceFile' module in REAL
    validated_comb = './scoring/validated_all_lib_comb'
    ...
    if nproc == 'Y':###Use default 70% of processors
        nproc = int(multiprocessing.cpu_count()*0.75)
...
##CL3v2.10 -> CLv2.20June 2013
##1. Added functionality to accept PARE tags with length higer tha tag_len parameter
##And chop longer PARE tags to tag_len - 'TagAbundanceFile' function
##
##2. Added make_index switch to help in re-running analysis - Saves time
##3. Added a switch to select Local vs Web analysis - Rev mapping is turned off for local analysis - same switch cab used to control procesing of FASTA files OR download from DBs

###CL3v2.20 -> CL3v2.25
###Added sorting of cleaveland predicted results on basis of p-value before making Uniq
####Added a local switch to choose local/mqsql mode of analysis
#### reported number sequences after bowtie mapping == 20n
#### Added swithch for Target prediction or not
####Fixed core issue with reverse mapping
