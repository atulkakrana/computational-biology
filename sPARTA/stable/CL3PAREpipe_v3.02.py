...
def GetCoords(con,db):
    ...
    ###########################################################TEST LINE#################################################################
    #cur.execute("SELECT chr_id,strand,gene,start,end,type FROM %s.gene_master WHERE type NOT LIKE 'mirna' AND gene NOT LIKE 'BLAST%%' AND chr_id = 1 ORDER BY chr_id,strand,start" % (db))###extra % escapes the % in query
    #####################################################################################################################################
    ...
    for entry in genome_info_inter_sort:
        ...
        if (entry[5] == 'miRNA' or entry[5] == 'tRNA' or entry[5] == 'rRNA' or entry [5] == 'snoRNA'):
            pass
        else:
...
if __name__ == '__main__':
    ...
    #only_libs = [(574,)] ## Supplied by 'TagAbundanceFile' module in REAL
