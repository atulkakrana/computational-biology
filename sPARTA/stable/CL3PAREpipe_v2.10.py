...
##2. A miRNA File specified by user - If file input used than table input will be turned off automatically
miRNA_file = 'miRinput.fa'##The header will be cleaned automatically
miRCode = 'ath'

##3. PARE database names
PAREdb = 'AT_sbsPriv_sRNA_v4'
...
##8. TargetFinder score cutoff (max allowed/Default is 7)
cutoff = str(7)

##9. Server with PARE data
dataserver = 'raichu.dbi.udel.edu'####Value should be in ''

##10. Server to upload data a.k.a destination server with table to hold results of PARE validation - Not for day to day analysis
...
##14 B. miRNA code as in mirBASE - The script will fetch miRNAs from mirBASE table in mir_master - For Lazy people - Like ATUL
miRTable = 'mirBASE'
miRCode = 'ath'

##14.  Not required if Local miRNA file used
...
def GetCoords(con,db):
    ...
    ###########################################################TEST LINE#################################################################
    #cur.execute("SELECT chr_id,strand,gene,start,end,type FROM %s.gene_master WHERE type NOT LIKE 'mirna' AND gene NOT LIKE 'BLAST%%' ORDER BY chr_id,strand,start" % (db))###extra % escapes the % in query###Limiting number of entries will effect last gene coords and sequence
    #####################################################################################################################################
...
def RevMapCoord(ent): ####DOES THIS MODULE WORKS IF USER USES MULTIPLE LIBRARIES - Parallelize
    ...
    if small_win != 0:
        rev_mapped_entry = ('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % (ent[0][1:],gene_name,chr_id,strand,new_bind_site, new_cleave_site,ent[4],ent[6],ent[7],ent[5],small_win,large_win,round(small_win/large_win,2)))
