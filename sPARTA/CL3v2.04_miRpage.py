...
####Parallelized version of RevMapCoord - introduced in version 2.04
def RevMapCoord(res_entry): ####DOES THIS MODULE WORKS IF USER USES MULTIPLE LIBRARIES - Parallelize
    ...
    if small_win != 0:
        rev_mapped_entry = ('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % (ent[0][1:],gene_name,new_bind_site, new_cleave_site,ent[4],ent[6],ent[7],ent[5],small_win,large_win,round(small_win/large_win,2)))
...
def main(): ###Variables need to be initialized in the ----main-------
    ...
    for res in fh_in:
        res_strp = res.strip('\n')
        ScoInpExt.append(res_strp)
    print ('**Listfrom ScoInpExt ready to feed to RevMapCoord function**')
    
    ###PARE summarization file for large and small window
    ...
    ##File to write results with new genomic co-oordiates
