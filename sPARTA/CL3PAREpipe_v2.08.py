...
##14. miRNA code as in mirBASE - The script will fetch miRNAs from mirBASE table in mir_master - Not required if Local miRNA file used
miRCode = 'mtr'
...
def miRinput(con):
    ...
    try:## If user specified some local miRNA file in config
        miRNA_file
        if True: ##Than process it and make a list
            ...
            for i in mir_blocks[1:]:
                ...
                miRs.append('%s,%s,%s,%s' % (ent[0],'None','None',ent[1]))## None at position 2 and 3 because so that name and seq at same index when data collected from table
