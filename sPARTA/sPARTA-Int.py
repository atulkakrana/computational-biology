...
## sPARTA-Int v1.19 - miRferno reached End of support -- Atul

#### PYTHON FUNCTIONS ##############################
import sys,os,re,time,timeit,csv,glob,string,shutil,operator,bisect,numpy,datetime,pyfasta
...
GenomeDB            = 'CICER_LIS1_genome'            ## DB to get fastaOut file
PAREdb              = 'CICER_sbsML_PARE'            ## Make sure that your Library is in the DB 

gffFile             = ''         ## From Phytozome - Not used if Local == N
genomeFile          = ''                  ## From Phytozome - Not used if Local == N

miRNA_file          = 'car-miRNAs.fa'                           ## If not present in directory than miRcode (below) will be sued to fetch miRNAs from miRbase table
miRCode             = 'None'                             ## Code of species to fetch miRNAs. Not used if miRNA file is present in directory
...
getPARELib          = 'Y'                               ## PARE libraries are present on Disk 'N' or need to be fetched 'Y'
libs                =['3936','3937','3938','3939','3940','3941','3942']                    ## Name of PARE libraries - MANDATORY
...
##
tag2FASTAstep       = 'Y'                               ## Convert PARE tag count file to FASTA file
map2DDstep          = 'Y'                               ## Map PARE reads to degradome
predStep            = 'Y'                               ## Reza functions
revMapstep          = 'Y'                               ## Reverse map to match co-ordinates with our genome
uniqRes             = 'Y'                               ## Combine results to a single file and filter out redundant entries
...
def getFASTAServer(con,db,gene_coords):
    ...
    for i in gene_coords: ###For every entry either gene or intergenic, use gene_info for just genes
        ...
        if tuple(i[0:2]) not in chromo_mem: ### Add chromosome and strand to list and fetch in memory, will be executed first time a unique chr_id and strand found
            ...
            print('Fetching gene %s to prepare FASTA file' % (gene))
            gene_seq = chromo[0][0][start:end].translate(str.maketrans("autgcn","AUTGCN")) ## make everything upper case, for uniformity across script, get first gene on that strand, 
            ncount = gene_seq.count('N')
            if ncount < len(gene_seq): 
                ...
                print("The seqeunce is purely made of 'N's")
                pass
       
                
        else: ## If chr_id is in chromo i.e. its loaded in memory just splice your sequence
            print('Fetching gene %s to prepare FASTA file' % (gene))
            gene_seq = chromo[0][0][start:end].translate(str.maketrans("autgcn","AUTGCN")) ## make everything upper case, for uniformity across script
            ncount = gene_seq.count('N')
            if ncount < len(gene_seq): 
                if strand == 'c':
                    gene_seq_rev = gene_seq[::-1]
...
def RevMapCoord(ent): ####DOES THIS MODULE WORKS IF USER USES MULTIPLE LIBRARIES - Parallelize
    ...
    if Local == 'N':
        ...
        if extraAnno == 'Y':
            ...
            cur.execute("SELECT cluster_id,phase_len FROM ASPARAGUS_UGA1_genome.phased_loci where chr_id = %s and (%s between start and end)" % (chr_id,int(new_cleave_site)))
            info2 = cur.fetchall()
...
def main():
    ...
    if revMapstep == 'Y':
        ...
        ### Make a dictionary of gene strands #########################################
        if Local == 'N':
            global nproc
        ...
        ##### Read the validated resuts file and cache entries #######################
        ...
        for afile in ResFls:
            ...
            for res in fh_in:
                ...
                acount+=1
            print ("** %s entries cached from '%s' for RevMapCoord function**" % (acount,afile))
            ...
            # ### FOR TESTING PURPOSE - SINGLE CORE ####################################
            # ValidTarGeno = []
            # print ('**Reverse mapping initiated in serial mode**\n\n')
            # for i in ScoInpExt:
            #     z = RevMapCoord(i)
            #     ValidTarGeno.append(z)
            # print ('**Reverse mapping complete for:%s\n\n\n' % (afile))
            # ### END OF TEST CODE ## USE PARALLEL MODE (below) for normal run #########
            
            ## PARALLEL MODE - Mutually exclusive with TEST MODE ABOVE #############  
            print ('**Reverse mapping initiated in parallel mode**\n\n')
            ValidTarGeno = PPResults(RevMapCoord, ScoInpExt) ## Results are in form of list
            print ('**Reverse mapping complete for:%s\n\n\n' % (afile))
            
            ## Filter for error 13-13-13 that is small window abundance = 0 and ratio calculation error
            ## If all validated targets gets filtered out of this error then that means the PARE Db corresponds to different genome
...
## v1.16 -> v1.17
## Minor bug fixed in revmapping, which used to stop after first entry - It was a loose exit() command.
