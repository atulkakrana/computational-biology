...
def mapdd2trans(anIndex):#
    ...
    retcode2 = subprocess.call(["bowtie2", "-a", "--end-to-end", "-D 1", "-R 1", "-N 0", "-L 20", "-i L,0,1","--score-min L,0,0","--norc","--no-head", "--no-unal", "-t","-p",nthread2, "-f", indexLoc, dd_file,"-S",map_out]) ###
    ## Optimized | no-unal was the culprit | 
    #retcode2 = subprocess.call(["bowtie2", "-a", "--end-to-end", "-D 1", "-R 1", "-N 0", "-L 20", "-i L,0,1","--score-min L,0,0","--norc","--no-head", "-t","-p",nthread2, "-f", indexLoc, dd_file,"-S",map_out]) ##
...
def PP(module,alist):
    ...
    start = time.time()
    nprocPP = round((args.accel/int(nthread))+1) 
    print('\nnprocPP:%s\n' % (nprocPP))
...
def main():
    ...
    if args.fileFrag:
        ...
        fragList = [file for file in os.listdir() if re.search(r'.*\.[0-9].*\.fa', file)] #
