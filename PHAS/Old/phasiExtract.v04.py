...
coordsfile      = "Table1_24PHAS_AbunAnno.txt"                      ## File with either phased loci ID 'Phas-100_w_7:6353898:6354260' or coords 'w,7,6353898,6354260'
head            = 'Y'
...
def writer(resList,con):
    ...
    for ent in resList: ## entry corresponds to one phased loci
        ...
        for i in phasiList:
            ...
            if fetchMax == "Y":
                if int(i[3]) == int(phase): ### Size specified in settings
                    cur.execute("SELECT tag,max_norm FROM %s.%s where tag = '%s'" % (DB,tagPosTable,tag))### Convert intergenic to gene name so as to get strand
                    info = cur.fetchall()
                    print("From tagpos table",info)
                    atag,max_norm = info[0]
                    abunList.append((atag,max_norm))
            else:
                # print("No need to catch max abundant phasiRNAs")
                pass
        ...
        if fetchMax == "Y":
            ...
            fh_out2.write("%s\t%s\t%s\n" % (phasID,abunList_sort[0][0],abunList_sort[0][1]))
