...
######## Developer settings ####################
minNoiseLen = 21        ## Recommended: 21
maxNoiseLen = 24        ## Recommended: 24
...
def chart(trans5,trans3,anormclust,bnormclust):
    ...
    for i,j in zip(bphasx_marker,bphasy_marker):
        plt.plot((i,i),(0,j),linestyle='--',linewidth=0.2, color='#454545',) ## plt.plot((x1, x2), (y1, y2), 'k-')
...
def mergeRNAs(trans,sRNADict,phasedDict,fastaDict,cur,finalLibs):
    ...
    will be retrived from sRNA mappings. For phasiRNA abundances will be fetched from the provided libraries. And for sRNAs (noise) library -specific abudnances from mapping will
    be used
    '''
...
def main():
    if runMode == 0:
        ...
        if fetchMap == 1:
            ...
            print("Please input correct mode for 'fetchmap'")
            sys.exit()
