...
#### Settings
genomeDB = 'ASPARAGUS_UGA1_genome' ##Used to identify strand of target
dataserver = 'raichu.dbi.udel.edu'

PAREpval = 0.25
PAREresType = 'M' ### M for MPPP and C for CL3
PAREres = 'ALL_Genic_Inter_revamapped_Uniq_miRMainAbun19Dec.csv' ## Has to be revmapped file with 14th column as cleave site
#PAREres='sco_inp_ext_geno'

phase = 21 ## Which phase you are using for double validation - Give corresponding file below
PHASresType = 'PP' ##J: jixian and P:Ping list file ##PP - Phas redundant script generated *.ping format
PHASres = '21_Final_PHASLociID_0.001_21ALL.ping'
pVal = '0.001' ## P-value of phased loci to test for
...
def validatePHAS(resList,PHASdict,phase,con,header):
    ...
    for ent in resList: ##ent format: miRNA,Target,cleavesite,whole entry
        ...
        ##PHAS LOCI MATCH - Fuck you were not cheking for cleave site in exact chromosme - Thats why you got shit load of results where there are scaffolds
        for akey in PHASdict.keys():
            ...
            if cleaveSite in aval:
                    if akey.split('-')[0] == chrid: ## I.e. clevage site is on same chromosme
                        print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite,tarStrand[0][0],akey,aval,(aval.index(cleaveSite)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
