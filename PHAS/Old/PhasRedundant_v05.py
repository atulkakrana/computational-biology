    ...
    for afile in fls: ###
        ...
        ##Second and further Instance - Match the entries of new file with dictioary and add new one#############
            ...
            for ent in lines:
                ...
                #print('Decode',mainkey_decode[0])
                #print ('Decode: %s-%s-%s' % (mainkey_decode[0].strip()[0],mainkey_decode[0].strip()[1:],mainkey_decode[-1].strip()[1:]))
                #mainkey_remade = '%s-%s-%s' % (mainkey_decode[0].strip()[0],mainkey_decode[0].strip()[1:],mainkey_decode[-1].strip()[1:])
                #print('Remade',mainkey_remade)
                ###################################################################################################################################
                
                #print('mainkey2:',mainkey.split('[')[2].replace("'","").replace(" ","").split(','))
                mainkey_decode = mainkey.split('[')[2].replace("'","").replace(" ","").split(',')
                #print('mainkey2:',mainkey_decode[2],mainkey_decode[3],mainkey_decode[4])
