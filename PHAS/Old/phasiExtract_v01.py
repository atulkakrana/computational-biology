...
coordsfile = 'Testcoords.csv' #### File with either phased loci ID 'Phas-100_w_7:6353898:6354260' or coords 'w,7,6353898,6354260'
coordsep =',' ### Seprator used in the file
phasedID = 'Y'### If given than phas ID is used to extract coords, if not than coords specified by user are used
...
clustfile = '2437.txt.score_p1e-07_sRNA_21_out.cluster'
phasiLenFilter = 'Y'
phassize = '21'
...
def getClust(clustfile):
    ...
    for ent in fh_in:#### Given an entry in coords file
        ...
        if phasedID == 'Y': ## Extract coords from phased ID
            ...
            get_chr_id = coords[1]
            get_start = int(coords[2]+stratbuff)
        ...
        for aclust in clusters[1:]:
            ...
            if round(sm.ratio(),2) >= 0.75: ### Get phasiRNA from this cluster
