...
clustfile = 'AllLibs.score_p0.0001_sRNA_24_out.cluster' ## Cluster file from phasing analysis, coul dbe for one lib or concatanated file fro all libs
    ...
    for ent in fh_in:#### Given an entry in coords file
        ...
        for aclust in clusters[1:]:
            ...
            if int(get_chr_id) == int(chr_id):
                ...
                if round(sm.ratio(),2) >= 0.99: ### Get phasiRNA from this cluster
        ...
        if finalMatchList:
            if phasiLenFilter == 'Y': ### If tags filter is ON
                ...
                for i in finalMatchList[:-1]: ## Last entry holds other info - phasiCyc, phasiSig,phasID and Clust_id
                    if int(i[-1]) == int(phase): ### Size specified in settings
        ...
        allMatchCount += matchCount ## Add the mached luster for each entry
        if matchCount > 0 :
            uniqMatchCount+=1
        else:
            print("####################################################################### NO MATCH FOUND")

    print ("Phased loci in input file:%s | Uniq matched cluster: %s | Total matched clusters found:%s" % (phasCount,uniqMatchCount,allMatchCount))
