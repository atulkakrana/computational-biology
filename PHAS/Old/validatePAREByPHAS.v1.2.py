...
PAREres     = 'All.genic.inter.revmapped_Uniq.csv'             ## Mode = 0 revmapped file with 14th column as cleave site | Mode: 1 -local validation file with cleave site at column 9
...
def validatePHAS(ent):
    ...
    if PAREresType == 'T':
        ...
        print("-Scanning phased loci for these cleave sites:",mirName,tarName,cleaveSite1,cleaveSite2,chrid,strand)
    else:
        cleaveSite = int(ent[2])    ## PARE supported cleave site
        print("-Scanning phased loci for this cleave site:",mirName,tarName,cleaveSite,chrid,strand)
    ...
    if mode == 0:           ## targets coordinates match to genome DB i.e. server target analysis was done
        ...
        if PAREresType == 'S' or PAREresType == 'C':
            ...
            for akey in PHASdict.keys():
                ...
                # elif cleaveSite3 in aval:
                #     if akey.split('-')[0] == chrid: ## i.e. clevage site is on same chromosme
                #         print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite3,tarStrand,akey,aval,(aval.index(cleaveSite3)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                #         matEnt = ('%s,%s,%s,na' % (ent[5],akey,(aval.index(cleaveSite3)+1-6)))

                # elif cleaveSite4 in aval:
                #     if akey.split('-')[0] == chrid: ## i.e. clevage site is on same chromosme
                #         print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite4,tarStrand,akey,aval,(aval.index(cleaveSite4)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                #         matEnt = ('%s,%s,%s,na' % (ent[5],akey,(aval.index(cleaveSite4)+1-6)))
                    ...
                    if offset == 1:
                        ...
                        # elif cleaveSite3 in aval1:
                        #     if akey.split('-')[0] == chrid: ## i.e. clevage site is on same chromosme
                        #         print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite3,tarStrand,akey,aval,(aval1.index(cleaveSite3)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                        #         matEnt  = ('%s,%s,%s,do' % (ent[5],akey,(aval1.index(cleaveSite3)+1-6)))

                        # elif cleaveSite4 in aval1:
                        #     if akey.split('-')[0] == chrid: ## i.e. clevage site is on same chromosme
                        #         print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite4,tarStrand,akey,aval,(aval1.index(cleaveSite4)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                        #         matEnt = ('%s,%s,%s,do' % (ent[5],akey,(aval1.index(cleaveSite4)+1-6))) 
                        ...
                        # elif cleaveSite3 in aval2:
                        #     if akey.split('-')[0] == chrid: ## i.e. clevage site is on same chromosme
                        #         print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite3,tarStrand,akey,aval,(aval2.index(cleaveSite3)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                        #         matEnt = ('%s,%s,%s,do' % (ent[5],akey,(aval2.index(cleaveSite3)+1-6)))

                        # elif cleaveSite4 in aval2:
                        #     if akey.split('-')[0] == chrid: ## i.e. clevage site is on same chromosme
                        #         print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite4,tarStrand,akey,aval,(aval2.index(cleaveSite4)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                        #         matEnt = ('%s,%s,%s,do' % (ent[5],akey,(aval2.index(cleaveSite4)+1-6)))
                        ...
                        # elif cleaveSite3 in aval3:
                        #     if akey.split('-')[0] == chrid: ## i.e. clevage site is on same chromosme
                        #         print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite3,tarStrand,akey,aval,(aval3.index(cleaveSite3)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                        #         matEnt = ('%s,%s,%s,so' % (ent[5],akey,(aval3.index(cleaveSite3)+1-6)))

                        # elif cleaveSite4 in aval3:
                        #     if akey.split('-')[0] == chrid: ## i.e. clevage site is on same chromosme
                        #         print('\nmiRNA:%s | Target:%s |CleaveSite:%s | Strand:%s @ PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite4,tarStrand,akey,aval,(aval3.index(cleaveSite4)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                        #         matEnt = ('%s,%s,%s,so' % (ent[5],akey,(aval3.index(cleaveSite4)+1-6)))
    ...
    ############################################################
        ...
        #####################################################
            ...
            for akey in PHASdict.keys():
                ...
                # elif cleaveSite3 in aval:
                #         if akey.split('-')[0] == chrid: ## i.e. clevage site is on same transcript
                #             print('\nmiRNA:%s | Target:%s |CleaveSite:%s | PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite3,akey,aval,(aval.index(cleaveSite3)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                #             matEnt = ('%s,%s,%s,na' % (ent[5],akey,(aval.index(cleaveSite3)+1-6)))
                # elif cleaveSite4 in aval:
                #         if akey.split('-')[0] == chrid: ## i.e. clevage site is on same transcript
                #             print('\nmiRNA:%s | Target:%s |CleaveSite:%s | PHAS:%s - %s phase: %d' % (mirName,tarName,cleaveSite4,akey,aval,(aval.index(cleaveSite4)+1-6))) ## 1 added to correct the index in human format and 6 added because start is at 6th postion
                #             matEnt = ('%s,%s,%s,na' % (ent[5],akey,(aval.index(cleaveSite4)+1-6)))                            
...
## In PHASredundant output p-values are checked as float that is all p-values less then cutoff will considered
