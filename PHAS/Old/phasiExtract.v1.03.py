...
clustfile       = "all.lib.txt.score_p0.0005_sRNA_24_out.cluster"  ## Cluster file from phasing analysis, coul dbe for one lib or concatanated file for all libs
fetchMax        = 1                                                 ## Fetch the max abundance phasi from tag position summmary, write in separate file
fetchLibAbun    = 1                                                 ## 0: Fetch libwise abundances for tags with 'phase' len required fpr heatmaps and other comparision between loci | 1: All tags from phaster, required to see abundance od phasiRNA tag and tags against all tags in these loci. The latter would be sum of abundances from all libs computed manualy in excel
sRNADB          = "ASPARAGUS_privPHAS_sRNA"                               ## if fetchMax == Y | Use this DB to get tag with max abundance

#### User specifed coords in input file
coordsfile      = "Final_24PHAS_Loci_0.0005_ALL.rocket.v3.csv"                   ## File with either phased loci ID 'Phas-100_w_7:6353898:6354260' or coords 'w,7,6353898,6354260'
...
excludeLibs     = [3379,3381,3402,3403,3404,3405,3416,3417,3418,3419,3420,3421,3422,3423,4590,4591,4592,4593,4594,4596,4597,4598,4599,4600,4601,4602,4603,4604,4605,4606,4682,5013,5015,5017]   ##  Used if fetchMax == 'Y' | Libs you wish to exclude, your tag position summary table should have libs ids and not lib codes
...
def writer(resList,con):
    ...
    if fetchMax == 1: ### Fetch max phasi for each loci 
        ...
        fh_out2.write("loci\tmaxtag\tmaxTagAbun\tmaxtag2\tmaxTagAbun2\ttotalPhasAbun\t#phaseTags\t%s\n" % ('\t'.join(x for x in libsHead)))
    ...
    for ent in resList: ## entry corresponds to one phased loci
        ...
        if fetchMax == 1:
            ...
            fh_out2.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (phasID,maxTag,maxAbun,maxTag2,maxAbun2,totalAbun,str(len(abunList_sort)),'\t'.join(str(x) for x in libAbunSum)))
...
## v1.02 -> v1.03
## In case of really p-value predictions, sometimes there is just one tag of phassize, in such cases fetchMax does not have second best tag and gave error. Now if second max tag is not available results will have it as "na" and 0 abundance