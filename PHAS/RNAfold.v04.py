...
### USER VARIABLES
runMode         =  'A'              ## A: Automatic - Provide list of ids, FASTA file which has precursors, these will extracted aotomatically 
                                    ## and phasiRNAs file from phasiExtract. phasiRNAs for corresponding transcript will be extracted automatically
                                    ## M: Manual mode provide FASTA file with single eprecursor and phasiRNA file with corresponding phasiRNA, both should in FASTA format

idFile          = 'IRcands.v3.list'         ## Required for mode 'A'
fastaFile       = '24Phas.fa'                 ## Required for mode 'A'
phasiFile       = 'All.txt.score_p1e-07_sRNA_24_out.clusterthres_0.25_phasi.csv'
mode            = 0                 ## 0: match direct reads only, 1: Match rev comp too, 3: Try to match direct, rev comp and comp
cleanup         = 1                 ## Delete temp files
lenFilter       = 0                 ## 0: Use all provided or extracted tags | 1: Use tags for specific lengths (lensThres)
lenThres        = [24,25]

#### FUNCTIONS ###########
def fastaReader(fastaFile):
    '''Read FASTA and give a list of tuples with name and seqeuences'''
    print("Fn: fastaReader ###############\n")
    ...
    print("Here is an example of FASTAList:",fastaList[1:5])
...
def fetchPhasi(anid,phasiList):
    ...
    for i in phasiList:
        ...
        if name.startswith(anid):
            ...
            if lenFilter == 1 and tagLen in lensThres:
                fh_out1.write(">%s\n%s\n" % (name,seq))
                acount +=1
                ...
                #### Do same as above but for all sizes 
                fh_out1.write(">%s\n%s\n" % (name,seq))
                acount +=1
                ...
                print("Please input correct mode for 'tagLen'")
                sys.exit()
...
def singlePlot(LONG_SEQ_FILE,SHORT_SEQ_FILE):
    ...
    print (longList)
    ent         = longList[1].split("\n")
    query       = ent[0].split()[0].strip()
    seq         = ''.join(x.strip() for x in ent[1:]) ## Sequence in multiple lines
    print(query,seq)
...
### v02 -> v03 Fixed handing of multiline fasta in Manual mode

### v02 -> v04
### Added lnegth filter to exclude lengths as in case of phasiRNAs
