...
def PHASBatch2(aninput):
    ...
    if runType == 'Y':### Uses Whole genome as input
        ...
        if deg == 'Y':
            ...
            if libFormat == "T":
                aformat = "t"
                retcode = subprocess.call([perl, "%s/phasiRNA_prediction_pipeline.ver.genome.pl" % (phaster_path), "-i", pro_file,"-f", aformat, "-t", sRNAratio,"-n", noiseLimit, "-d", index, "-px", out_file, "-rl", rl, "-cpu", nthread])
            elif libFormat == "F":
                aformat = "f"
                retcode = subprocess.call([perl, "%s/phasiRNA_prediction_pipeline.ver.genome.pl" % (phaster_path), "-i", pro_file,"-f", aformat, "-t", sRNAratio,"-n", noiseLimit, "-d", index, "-px", out_file, "-rl", rl, "-cpu", nthread])
            else:
        ...
        if deg == 'Y':
            ...
            if libFormat == "T":
                aformat = "t"
                retcode = subprocess.call([perl, "%s/phasiRNA_prediction_pipeline.ver.MUL.pl" % (phaster_path), "-i", pro_file, "-f", aformat, "-t", sRNAratio,"-n", noiseLimit, "-d", index, "-px", out_file, "-rl", rl, "-cpu", nthread])
            elif libFormat == "F":
                aformat = "f"
                retcode = subprocess.call([perl, "%s/phasiRNA_prediction_pipeline.ver.genome.pl" % (phaster_path), "-i", pro_file,"-f", aformat, "-t", sRNAratio,"-n", noiseLimit, "-d", index, "-px", out_file, "-rl", rl, "-cpu", nthread])
            else:
