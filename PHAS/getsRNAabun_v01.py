...
CompareDBs = 'Y' ## Fetch abundance from two tables each for one DB for comparision
conTable = '`RICE_drt_sRNA_summary2`'
db = 'kakrana'
server = 'erawan.dbi.udel.edu'
tagfile = 'LDMAR_PARE_valid_phasi.seq' ### Text file with tags
...
def tags2Abun(con,tagfile,testTable,conTable):
    ...
    ##Write file header
    if CompareDBs  == 'Y':
        fh_out.write('tag,NormSumRatio,TestNorm,ConNorm,%s,%s\n' % (testTablecol,conTablecol))
    else:
    ...
    for i in fh_in:
        ...
        if CompareDBs  == 'Y':
            ...
            if temp:## Tag is found 
                ...
                conNormSum = temp[0][4]
                fh_out.write('%s,%s,%s,%s,%s,%s\n' % (tag,round((testNormSum/conNormSum),2),testNormSum,conNormSum,testAbundances,conAbundances))
                ...
                conNormSum = 0 ## not temp[0][4] like above
                fh_out.write('%s,%s,%s,%s,%s,%s\n' % (tag,round((testNormSum-conNormSum),2),testNormSum,conNormSum,testAbundances,conAbundances)) ## NormAbundance is substracted to avoid division to zero error
