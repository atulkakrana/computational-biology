...
runMode     = 1                                             ## 0: Full run - find candidates, validate candidates 1: finalPairs file exists from earlier run now lib fetching, mapping, and plot files need to be generated 
                                                            ## 2: All earlier file exists, only consensusCands are new
fetchMap    = 0                                             ## 0: Do not fetch or map 1: Fetch libs and map to fasta file, madatory for runMode 0 
foldback    = 1                                             ## Boolean [0 and 1]. 1: Procedes with only those that are self foldback 0: Proceed with best match between a pait of transcripts (was default mode)
phase       = 24
fastaFile   = "AO.24Phas.collaspsed.clean.fa"
blast_rc    = "Final_BLAST_RC.txt"
blast_nor   = "Final_BLAST_NOR.txt"
blast_comp  = "Final_BLAST_COMP.txt"
phasedRes   = "24PHAS_p0.005_collapsed.minlen150.csv"
clustfile   = "24PHAS_p0.005_clust-libcombraw-libwise.formatted.txt"       ## Cluster file from phasing analysis, coul dbe for one lib or concatanated file for all libs
phasMode    = "O"                                            ## O: Observed phasiRNAs positions or G: guessed phasiRNA position begining from first phasiRNAs
db          = "ASPARAGUS_privPHAS_sRNA"                      ## sRNA DB
fetchLibIDs = "N"                                            ## (Y): Get IDs for all libs in DB (N): If you want to run on specific libs than 'N' and specifiy libs below
userLibs    = [(50135014,),]                                     ## If fetchLib = 'N' then specifiy library IDs in the format [(lib1,),(lib2,),(lib3,),]

consensusRes = 'Y'
consensusCands = "IRcands.list"
matchThres   = 0.35
libType      = 0                                                 ## 0: Lib_ids (4518) | 1: lib_code ('leaf_1')
excludeLibs  = [3403,3404,3405,4599,4600,4601,4602,4604,4605,4606,4682,4593,4597,4598,3416,3418,3419,3421,3422,5013,3379,3381,4591,4592,4596,3417,3423,4589,5017,4588] ## Included n = 9 
# excludeLibs  = [3403,3404,3405,4599,4600,4601,4602,4604,4605,4606,4682,4593,4597,4598,3416,3418,3419,3421,3422,5013,3379,3381,4591,4592,4596,3417,3423] ## Included n = 12

######## Developer settings ####################
noiseAbun   = 0         ## 0: Used from the library file provided for mapping [default] | 1: fetched from server using libraries provided above i.e. same as phasiRNAs
...
server      = "raichu.ddpsc.org"             ## Server to use to fetch library information and smallRNA libraries
numProc     = 58
...
def inferIRs(list_rc,header,phasSet):
    ...
    for i in rc_sort:
        ...
        if (trans5 not in candidateList) and (trans3 not in candidateList) and length >= 180 and pid >= 85.00 and length >= minLen:
            ...
            if foldback == 1:
                ## Give priority to only the self foldbacks, and not with others
                if trans5 == trans3:
                    ...
                    ## SKIP these as user is not interested in interference from other PHAS loci
                    ## This is important because sometimes foldbacks are not reproted with self due to complentatrity with others
                    pass
            else:
                uniqList.append((trans5,trans3,pid,bitscore,hang5,hang3,match,"IR",i))
                fh_out.write("%s\tIR\n" % ("\t".join(z for z in i)) )
...
def validateIR (clustfile,uniqList,phasList,fastaFile,header,phasSet):
        ...
        for aphas in trans5Phas:
            for bphas in trans3Phas:
                ...
                if acoords:
                    ...
                    print("Halt - No coordinates reported for aphas:",aphas)
                    # sys.exit()
                    continue
                ...
                if bcoords:
                    ...
                    print("Halt - No coordinates reported for bphas:",(bphas))
                    # sys.exit()
                    continue
    ...
    print("Len of validList:%s" % (len(validList)))
    # sys.exit()
...
def prepareChartFiles(validList,clustFile,userLibs,fastaFile,con):
    ...
    for lib in userLibs:
        ...
        directionInput  = "%s.consensus.input.direction.txt" % (lib)
        fh_out3         = open(directionInput,'w')
        fh_out3.write("pairname\tPHAS-5arm\tPHAS-3arm\tabun5-side\tabun3-side\tabunratio\tstatus\n") ## combname,phasiname,phasistrand,phasiabun,phasiseq,phasilen,phasihits,phasiflag,phasipos,phasiarm
        ...
        for pair in validList:
            ...
            if astatus  == 'IR':
                ...
                trans3  = pair[0][1]
                akey    = "%s_%s" % (trans5,trans3)
                if akey not in processedSet:
                    processedSet.add(akey)
                    ...
                    if consensusRes == 'Y':
                        ...
                        processDirectionL               = inferDirection(aconsensusclust,bconsensusclust,aflag,bflag)
                        # sys.exit()
                    ...
                    chart(trans5,trans3,anormclust,bnormclust,IRcoords)
                    ...
                    if consensusRes == 'Y':
                        ...
                        ### Write results for processing direction prediction - name, abudance for first half of IR, abundance for second half of IR, log ratio of these abundances first against second half, and procesisng direction
                        fh_out3.write("%s\t%s\n" % (combName,'\t'.join(str(x) for x in processDirectionL)))
                else:
                    # print("## pair processed already - happens when you have multiple PHAS entries for same transcripts in PHAS list")
                    # print("## chart input files will have just one entry with phasiRNAs from all PHAS in PHAS list")
                    pass
...
def mergeIRs(consensusInput,consensusCands):
    ...
    anestlist = [[0,0,0,0,0] for x in range(maxLen+1)] ## Nested list with positions, and nest with 1 slot for 18-20 and 4 slots for 21,22,23,and 24nt size abundnaces
    bnestlist = [[0,0,0,0,0] for x in range(maxLen+1)] ## Nested list with positions, and nest with 1 slot for 18-20 and 4 slots for 21,22,23,and 24nt size abundnaces
        ...
        if combname in combnameSet: ## If pair includes a transcript in user input IR.list
            ...
            if apos <= maxLen: ## Record only those sRNAs or phased abundances whose position is till last phased siRNAs, no need to record sRNA when there is no phasiRNA on precursor 
                ...
                if xflag    == 'A':
                    ...
                    if alen > 17 and alen < 21: ## 18nt to 20nt sRNA abundances
    ...
    for i,j in zip(alist[1:],anestlist[1:]): ## Not include the 0 position of list, because there is no 0 position in IRs
        ...
        fh_out2.write("%s\t%s\t%s\t24\n" % (acount,j[4],flag)) ## File with abundance for all sizes merged
    ...
    for z,x in zip(blist[1:],bnestlist[1:]): ## Not include the 0 position of list, because there is no 0 position in IRs
        ...
        fh_out2.write("%s\t%s\t%s\t24\n" % (acount,x[4],flag)) ## File with abundance for all sizes merged
...
###### Helper Functions ########################
###############################################
def inferDirection(aconsensusclust,bconsensusclust,aflag,bflag):
    '''Infers direcion of processing by comapring abudnaces from first half of phases with rest'''
    
    print("\nFUNCTION: inferDirection")
    
    ## General dicts - Deprecated - Now positions corresponding to index are computed on fly - See variables aclustbreakpos5 and bclustbreakpos5
    # adict = {1:28, 2:52, 3:76, 4:100, 5:124, 6:148, 7:172, 8:196, 9:220, 10:244, 11:268, 12:292, 13:316, 14:340, 15:364, 16:388, 17:412, 18:436, 19:460, 20:484, 21:508, 22:532}
    # bdict = {1:25, 2:49, 3:73, 4:97, 5:121, 6:145, 7:169, 8:193, 9:217, 10:241, 11:265, 12:289, 13:313, 14:337, 15:361, 16:385, 17:409, 18:433, 19:457, 20:481, 21:505, 22:529}



    resL = [] ### Stores phas-tags in 5'-arm and 3'arm, abundance in first half of clusters, second half of cluster, log ratio of abudnance between both, result (5 or 3)

    ### Valid clusters i.e. those with at-least six phases with flip flags A or B and not N
    acount  = 0  ## Count number of phased siRNAs
    bcount  = 0  ## Count number of phased siRNAs
    aabun5  = 0  ## Abundance of first half (towards 5') subcluster
    aabun3  = 0  ## Abundance of second half (towards 3') subcluster
    babun5  = 0  ## Abundance of first half (towards 5') subcluster
    babun3  = 0  ## Abundance of second half (towards 3') subcluster
    aclust  = [] ## Stores a sublist 
    bclust  = [] ## Stores a sublist

    # print(aflag,bflag)
    if (aflag =="A" and bflag =="B") or (aflag == "N" and bflag=="N"): ### Avoid the aflag == "N" as their phas postions are akward, therefore these needs manual curation
        for ent in aconsensusclust:
            if ent[6] == "P":
                acount += 1
                aclust.append((ent[2],ent[4],ent[7])) ## Abun, tag len and position

        for ent in bconsensusclust:
            if ent[6] == "P":
                bcount += 1
                bclust.append((ent[2],ent[4],ent[7])) ## Abun, tag len and position

        print("--valid consensusflags in clusters - 5-arm:%s | 3-arm:%s" % (acount,bcount))

        asum = acount+bcount
        aclust = sorted(aclust,key=lambda x: int(x[2]),reverse=False)
        bclust = sorted(bclust,key=lambda x: int(x[2]),reverse=False)
        # print("aclust sorted",aclust)
        # print("bclust sorted",bclust)
        if (acount >=5 and bcount >= 5) or (asum >= 12): ## Cluster has required data to make a descion on processing end
            maxpos          = max(aclust[-1][2],bclust[-1][2])
            maxphasindex    = int(maxpos/24)
            clusterbreak    = int(maxphasindex/2)
            print("aclust max",aclust[-1]," | bclust max",bclust[-1], " | maxphasindex",maxphasindex," |clusterbreak",clusterbreak)
            # aclustbreakpos5 = adict[clusterbreak]
            # bclustbreakpos5 = bdict[clusterbreak]
            aclustbreakpos5 = (phase*clusterbreak)+4 ## Instead of a static dictionary now PHAS specific consensus positions computed on fly 
            bclustbreakpos5 = (phase*clusterbreak)+1 ## Instead of a static dictionary now PHAS specific consensus positions computed on fly

            if maxphasindex % 2==0 and maxphasindex !=0:
                ### Even phasindex, two equal breaks
                # aclustbreakpos3  = adict[int(clusterbreak+1)]
                # bclustbreakpos3  = bdict[int(clusterbreak+1)]
                aclustbreakpos3  = (phase*(clusterbreak+1))+4
                bclustbreakpos3  = (phase*(clusterbreak+1))+1
            else:
                ### Odd phas-index the middle needs to be not included in abudances
                # aclustbreakpos3  = adict[int(clusterbreak+2)] ### Consider total phases = 11, so clusterbreak = P5, but P6 needs to be skipped to balance for 5-side and 3-side PHAS positions, so P5+2 = P7, here we skip P6
                # bclustbreakpos3  = bdict[int(clusterbreak+2)] ### Consider total phases = 11, so clusterbreak = P5, but P6 needs to be skipped to balance for 5-side and 3-side PHAS positions, so P5+2 = P7, here we skip P6
                aclustbreakpos3  = (phase*(clusterbreak+2))+4 ### Consider total phases = 11, so clusterbreak = P5, but P6 needs to be skipped to balance for 5-side and 3-side PHAS positions, so P5+2 = P7, here we skip P6
                bclustbreakpos3  = (phase*(clusterbreak+2))+1 ### Consider total phases = 11, so clusterbreak = P5, but P6 needs to be skipped to balance for 5-side and 3-side PHAS positions, so P5+2 = P7, here we skip P6


            print("--Maxpos:%s | maxphasindex:%s | clusterbreak:%s | aclusterbreakpos5:%s | aclusterbreakpos3:%s | bclusterbreakpos5:%s| bclusterbreakpos3:%s" % (maxpos,maxphasindex,clusterbreak,aclustbreakpos5,aclustbreakpos3,bclustbreakpos5,bclustbreakpos3))
        
            ### Compute abundance for both sub-clusters and ratio of abundances
            for ent in aclust:
                if int(ent[2]) <= int(aclustbreakpos5)+4: ## 3 nt buffer for dicer offset
                    aabun5+=int(ent[0])
                elif int(ent[2]) >= int(aclustbreakpos3)-4 and int(ent[2]) <= maxpos:
                    aabun3+=int(ent[0])
                else:
                    pass

            for ent in bclust:
                if int(ent[2]) <= int(bclustbreakpos5)+3: ## 3 nt buffer for dicer offset
                    babun5+=int(ent[0])
                elif int(ent[2]) >= int(bclustbreakpos3)-4 and int(ent[2]) <= maxpos:
                    babun3+=int(ent[0])
                else:
                    pass
            ...
            abun5 = (aabun5+babun5)
            abun3 = (aabun3+babun3)
            xratio = float(math.log(abun5,2) - math.log(abun3,2))
            ...
            #### Result
            resL = [str(acount),str(bcount),str(abun5), str(abun3), str(xratio),str(status)]

        ### Do not have sufficient data to make calculations and a decision
        else:
            resL = [acount,bcount,"na","na","na","na"]
    
    else: ### NO data to make correct decision
        resL = ["na","na","na","na","na","na"]

    print("--Processing direction result:",resL)

    return resL

def chart(trans5,trans3,anormclust,bnormclust,IRcoords):
    '''
    ...
    start2      = int(IRcoords[0][8])
    end2        = int(IRcoords[0][9])
    aarmLen     = end1-start1+1
    barmLen     = end2-start2+1
    ...
    bOthers = plt.scatter(bOthersx, bOthersy, s=[n**2 for n in bOthersy], c='#383838', lw = 0, alpha=0.5)
    ...
    bphasy_marker = [min(bALLy)]*len(bphasx_marker) ## Most abundant value on y-axis multipled the times of x marker to get list of same size as bphasx_marker
    ...
    plt.title("First phase: %s(5') - %s(3') | Last phase: %s(5') - %s(3') | Total phase: %s(5') - %s(3') | Arm Length: %s(5') - %s(3')" % (aphasmin,bphasmin,aphasmax,bphasmax,len(aphasx_marker)-1,len(bphasx_marker)-1,aarmLen,barmLen),fontproperties=font_manager.FontProperties(size=7)) ## In figures 1 or 2 phases might be extra, these are noise sRNAs showing up in phass
...
def mapLibs(fastaFile,fetchedLibs):
    ...
    for alib in fetchedLibs:
        ...
        retcode = subprocess.call(["bowtie","-f","-a","-v",mismat,"-p", nproc2,"-t" ,fastaIndex, fastaFile, mapFile])
...
def map2Dict(alib):
    ...
    for i in mapRead:
        ...
        phasiname,phasiabun = aname.strip().split("|") ## Updated to match tag2Fasta v03
        ...
        # print(aname,phasistrand,transname,phasiabun,phasiseq,phasilen,phasihits,phasiflag,phasipos)
    ...
    for atrans in transSet:
        ...
        for i in mapList_s: ## For entry in mapped list
            ...
            if transname == atrans and matchFlag == 0: ## This is the first phasiRNA for this transcript
                ...
                value.append((aname,phasistrand,phasiabun,phasiseq,phasilen,phasihits,phasiflag,phasipos))
...
def clust2Dict(clustFile):
    ...
    for gettrans in transSet:
        ...
        print("-\ntrans name:%s | value snippet:%s" % (gettrans,valueuniq[0:3]))
...
def mergeRNAs(trans,sRNADict,phasedDict,fastaDict,cur,finalLibs):
    ...
    #         if noiseAbun    == 0: ## Use abudnances from the provided library FASTA file
    #             transClust.append((ent[0],ent[1],ent[2],ent[3],ent[4],ent[5],ent[6],int(ent[7])))
    #         elif noiseAbun  == 1: ## Fetch libraries from the server for same libaries as phasiRNAs
    #             atag,noiselibabun,lib_abun  = getAbundance(cur,srnaseq,finalLibs)
    #             transClust.append((ent[0],ent[1],int(noiselibabun),ent[3],ent[4],ent[5],ent[6],int(ent[7])))
    #         else:
    #             print("Wrong noiseAbun mode selected - check settings, script will exit")
    #             sys.exit()
    #     else:
...
def main():
    if runMode == 0:
        ...
        if fetchMap == 1:
            ...
            print("+Bowtie maps need to be present for libs %s specified in settings,else turn ON fetchMap" % (userLibs))
            pass
...
## V1.9 -> v2.0
## Added functionality to define the length used for plotting by min/maxPlotLen parameters, and added them to plots as grey color

## v2.0 -> v2.1
## Added few more information to the chart header

## v2.1 -> v2.11
## Fixed size issue of other sizes in chart module. Earlier these were being made of size same as 24-nt due to a type where the size equation is written

## v2.11 -> v2.2
## The noise sRNAs abundaces too are fetched from server for libraries specifed above, this is same as it was done for phasiRNAs in ealrier versions. The switch to control this nature is noiseAbun
## The 18 to 20nt sRNAs are now included in the consensus.merged files 

## v2.11 -> v2.3
## Added a new functional module inferDirection(aconsensusclust,bconsensusclust,aflag,bflag), which infers processing direction
## Fixed breaking PHAS into two clusters in case of odd number of PHAS pocition the one is at centre is discared from both

## v2.3 -> v2.4
## tested for genome extracted transcripts
## Added a check in chartinput to not process same pair again again just becaus ethere are multiple PHAS entries for same transcript
#### And reason for that is that prepareChartFiles - looks for all phasiRNAs and all sRNAs that match to a pair and then prepares
#### files for plotting. So, results from all different PHAS loci for that transcript are included in one
## Changed Bowtie mde form -n to -v
## In the function 'inferDirection', position corresponding to the PHASindex is computed on fly rather than using a static dictionary
## Added a check in inferIRs, to include only self foldbacks, it is controlled by 'foldback' switch
## Updated map2Dict to correctly parse tag name and abundance, as the header has been changed in tag2Fasta.v03
## Fixed a bug in map2dict where phasiname was being addded to dict but it was not being assigned in the loop
#### So name of last phasi from earlier loop ws being used for all noise sRNAs
