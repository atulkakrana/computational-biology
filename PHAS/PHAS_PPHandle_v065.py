...
geno_index = '/alldata/Genomic/Pineapple/UIUC/v3/BowtieGenomicIndexes/PINEAPPLE_UIUC3_genome'           ## If geno = 'Y' then index path from $ALLDATA; if geno = 'N' then prepare index of your file 
                                                    ## using bowtie-build command like bowtie-build GENOMEFILE.fa NAME_FOR_INDEX

## sRNA - Mandatory
db = 'PINEAPPLE_priv_sRNA' ## sRNA DB
fetchLibIDs = 'Y'                                   ## (Y): Get IDs for all libs in DB (N): If you want to run on specific libs than 'N' and specifiy libs below
userLibs = [(2435,),(2436,),(2437,)]                ## If fetchLib = 'N' then specifiy libraru IDs in the format [(lib1,),(lib2,),(lib3,),]
