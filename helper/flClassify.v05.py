...
######### USER INPUTS ###############
runmode         = 2                                                         ## 1. Full discovery mode | 2. Fetches promoter for transcripts that you believe are FL (names in single column provided to phasFile input )
...
phasFile        = "FL.target-based.ids"                         ## runmode 1: PHAS redundant file | runmode 2: single column file with transcripts ids (these should match named in GTF file) 
...
def findFL(featureFile,phasFile):
    ...
    if runmode == 1:
        ...
        ''' This mode is written for cases when you have a list of PHAS transcripts that you believe is already FL and
        just want to extract the promoters - Accepts input file with just phased ids, the names should match those in GTF file
        '''

        ### Read PHAS -FL ids file
        fh_in = open(phasFile,'r')
        flids = fh_in.readlines()

        ### Uniq set of all transcripts
        transS = set() 
        acount = 0 ### Total input ids
        for i in flids:
            acount +=1
            anid = i.strip("\n")
            transS.add(anid)
        print("Total unique ids %s | cached for promoter extraction:%s" % (acount,len(transS)))
        # print("Snippet of PHAS IDs:",transS[1:10])

        ### Get coords for your FL transcripts
        bcount = 0  ## Total IDs processed
        for aphas in transS:
            bcount+=1
            print("++phas id:%s" % (aphas))
            phastrans,phasexons = getFeatures(aphas,featureTable,conn)
            # print(phastrans)
            gname,tname,achr,astart,aend,astrand,atype,aflag = phastrans[0]
            flDict[tname] = (int(achr),astrand,int(astart),int(aend))

        print("Total PHAS IDs:%s | Coords Fetched:%s" % (bcount,len(flDict)))
...
def writer(longPrm,uniqPrm):
    ...
    ### Write long promoter
    outfile1 = "%s.longprm.txt" % (phasFile.rpartition(".")[0])
    ...
    outfile2 = "%s.uniqprm.txt" % (phasFile.rpartition(".")[0])
...
def getFeatures(phas,featuretable,conn):
    ...
    print ("Exiting function - getFeatures\n")
...
#### LOG
## v04 -> v05
## Added runmode 2 that allows direct input of PHAS ids that seem FL, i.e. already decided by user that these are FL. No attempt to infer tthese as FL done by script.



#### USAGE
#### 1. Full discovery mode - Accepts a PHAS file, extracts it's entries (transcripts and exons) from a GTF file, check for targets in these transcripts and exons, and finally computes promoter and writes in file. Two files with promoter cordinates are written - One with promoyer seqeunce for the leftest 5' start site and one with promoter starting from rightest site and going upstream 1kb or as requested

#### 2. Get promoters mode - Accepts a single column list of PHAS ids, you must have detrmined that these are FL using target prediction i.e. thise still have targets. Uses this list of ids, extracts transcriptst and exons from GTF file and finally computes promoter and writes to file.Two files with promoter cordinates are written - One with promoyer seqeunce for the leftest 5' start site and one with promoter starting from rightest site and going upstream 1kb or as requested
