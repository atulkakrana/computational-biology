...
##### INPUT #########################
mode            = 1                                                 ##  0: Fetch from tag position summary table or
                                                                    ##  1: Fetch from sRNA DB - Required when sRNA libraries are mapped to other genome and has no hits

tagfile         = 'tag.list'                                        ##  input text files with tags as one and only first column
doUniq          = 0                                                 ##  0: All tags as is 1: remove redundant tags, give resuts for uniq list

DB              = 'ASPARAGUS_privPHAS_sRNA'                         ##  Used with mode:1 - If abundance to be fetched directly from sRNA DB, required if no genome is available, and hit = 0

testTable       = 'lullu'          ##  Used with mode:0
...
def tags2Abun(con,tagfile,testTable,conTable):
    ...
    for i in fh_in:
        ...
        if temp: ## Tag exists in normalized table
            ...
            testAbundances = ",".join(str(x) for x in temp[0][5:]) ## in comma separated format, from fifth column onwards i.e abundance columns
...
def tag2Abun2(con,tagfile):
    ...
    for i in fh_in:
        ...
        if doUniq == 1:
            if uniqTags not in uniqTags:
                tag = i.strip('\n').replace('-','').translate(str.maketrans("U","T"))## Manipulation if tag is from PARE valid results - reversed and U -> T
                print("'%s' being queried:" % (tag))
                hits,rawAbun,normAbun = sRNAAbun(tag)
            ...
            tag = i.strip('\n').replace('-','').translate(str.maketrans("U","T"))## Manipulation if tag is from PARE valid results - reversed and U -> T
            print("'%s' being queried:" % (tag))
            hits,rawAbun,normAbun = sRNAAbun(tag,con,libs)
            ...
            print("Please selct correct mode for 'doUniq' function")
            sys.exit()
...
def sRNAAbun(tag,con,libs):
    '''Get tagabundance and hits from sRNA DB for 'tag2Abun2' module '''
    ...
    ## Get normalized and raw abundances
    for lib in libs:
...
## v02 -> v03 
## Added functionality to uniqTags and then fetch the abudances
## Separated function to get abundance from sRNAs Dbs to make script more managable