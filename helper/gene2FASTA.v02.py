...
seqType     = '1' ## 1 = CDS , 2 = Transcript (Minus introns, only model 1 of gene used), 3 = Complete gene + buffer (specify), 4 = 5' promoter of gene as specifed by buff5 below (including 5' UTR)
...
def genes2CDS(con,idFile,genomedb):
    ...
    acount = 0 ## Count not found genes
    bcount = 0 ## Count empty genes
    for anent in fh_in:
        anent_splt = anent.split(sep)
        gene = anent_splt[int(geneID)-1].strip()
        print ('\nExtracting sequence for ID: ',gene)
        
        ## Test
        # cur.execute ("SELECT * FROM %s.gene_position WHERE gene = '%s'" % (genomedb,gene)) ## Select a model if several available for gene for sequence extraction, T0 entries of a gene have negative model so removed from here
        # model = cur.fetchall()
        # print("Model:",model)
        # sys.exit()
        ...
        if cds != "":
            ...
            ## Seqeunce is empty maybe coords are not correct
            bcount+=1

    print("\nGenes not found:%s | Genes with empty sequence that were removed:%s" % (acount,bcount))
...
## v01 -> v02
## Added strip() to query gene, the gene ids from meyers website had a an empty space after gene id.
## Reports counts of those genes,cds and transcripts that are not found in DB and those with no sequence
