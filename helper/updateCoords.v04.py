...
######## SEETINGS ################
mode        = 1                            ## 0: Fetch FASTA from coords and revmap 1: FASTA file supplied just revmap
server      = 'raichu.ddpsc.org'         ## Server to use to connect to DB
genomedb    = 'ASPARAGUS_UGA1_genome'       ## Genome DB to use for extraction
newGenome   = "/home/kakrana/99.genomes/asparagus/v2/modified/AsparagusCHR_V1.1.fa"
...
SpecStrand  = 'Y'                           ## [mode 0] Y: If strand is available and sequence needs to be from specific strand | F: No strand information and you need forward strand | B: Than you dont care about strand and want from both - This mode is not used in this script
strandpos   = 58                            ## [mode 0] Column for strand - excel format - should be 'w' and 'c'

header      = 'Y'                           ## [mode 0] Does file have header
sep         = '\t'                          ## [mode 0] Specify seprator used in spreadsheed or input file
namepos     = 53
chrpos      = 55                             ## [mode 0] Column for chromosome - excel format
startpos    = 56                             ## [mode 0] Column for start postion- excel format
endpos      = 57                             ## [mode 0] Column for end position - excel format
...
def mapper(fastaFile,newGenome):
    ...
    if gmapDB == 1:
        ...
        newGenomeDB = "%s.db" % (newGenome.split("/")[-1].rpartition('.')[0])
        db_folder = "%s/gmap_temp" % (os.getenv('HOME'))
        print("Will use gmap DB %s in folder %s - Make sure it exists or run with gmapDb = 1" % (newGenomeDB,db_folder))
...
def uniqBED(coords,bedFile):
    ...
    if mode == 0:
        ...
        print("Please enter correct mode - System will exit")
        sys.exit()
...
def main(genomedb,coords):
    if mode == 0:
        ...
        fastaFile = coords ## The strandrad input is fasta file
...
## v03 -> v04
## Now takes fasta seqeunces too, in mode ==1, instead of coords