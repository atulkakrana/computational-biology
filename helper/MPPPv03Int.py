...
###Cleaned up extenal version of script v01
...
#### USER SETTINGS ################################
Local = 'N' ##External version will be used if Local is 'Y'
dataserver = 'raichu.dbi.udel.edu'
GenomeDB = 'AT_TAIR10_genome' ##DB to get fastaOut file
PAREdb = 'AT_pub_PARE_v4' ##Make sure that your Library is in the DB
...
miRNA_file = 'miRinput.fa'
miRCode = 'ath'
ver = '20'
miRTable = 'mirBASE'

genomeFeature = 0 ## 0 for gene and 1 for inter; 2 for both
libs =['65_chopped.txt','3630_chopped.txt'] ## Name of lib files
...
fragFasta = 'Y' ### Obsolete
indexStep = 'N'
tarPred = 'N' ## Target prediction required or not 'Y' and 'N'
TPmode = 'H' ## 'H' for heuristic and 'E' for exhaustive and 'R" for rebel
TPscore = 'N' ## R -Rebel and 'N' for normal
tag2FASTAstep = 'N'
map2DDstep = 'N'
...
def fragFASTA(FASTA):
    ...
    if filesize <= splitCutoff: ## No need to split file too small
        ...
        if seq_count >= 30: ## Check file is chromosme based or contig/cDNA based 
            ...
            splitnum = str(round(filesize/10))
            #splitsize = str(2) ## Number of files to split in, try to keep size close to 100MB
            #FASTA = '../'+FASTA
            if fragFasta == 'Y':
...
def tarFind3(frag):
    ...
    if indexStep == 'Y':
        ...
        retcode = subprocess.call(["/home/bioinfo/tools/bowtie2.1/bowtie2-build", frag, index])
    ...
    if retcode == 0: ### Index creation sucessful or index already exists
        ...
        if TPmode == 'H': ## Heurustic
            ...
            retcode2 = subprocess.call(["/home/bioinfo/tools/bowtie2.1/bowtie2","-a","--end-to-end","-D 3","-R 2","-N 1","-L 8","-i","S,4,0.5","--rdg","24,8","--rfg","12,8","--min-score","G,-20,-2","--norc","--no-unal","--no-hd","-p",nthread2, "-x", index, "-f" ,"miRinput_RevComp.fa","-S", file_out])
            ...
            refGap = str("12,8")
            #### Jan 13 -D 7 -> -D 4 | 
            retcode2 = subprocess.call(["/home/bioinfo/tools/bowtie2.1/bowtie2","-a","--end-to-end","-D 4","-R 2","-N 1","-L 6","-i",intervalFunc,"--rdg",readGap,"--rfg",refGap,"--min-score",minScoreFunc,"--norc","--no-hd","--no-unal","-p",nthread2, "-f", index, "miRinput_RevComp.fa","-S", file_out])
...
def tarParse3(targComb):
    ...
    fh_in = open('./temp/%s' % targComb,'r')
    targRead = fh_in.readlines()
    TarPred =  './temp/%s.parsed.csv' % (targComb) ### Similar to parsed target finder format
    ...
    #print('miRNA: %s\n%s' % (miRrevcomp[::-1],miRrevcomp[::-1].replace("T","U") ))
        ...
        if TPscore == 'R': ## Allowed 3 MM, 2 Wob, 1 Gap
            ...
            for i in gap:
                score += 1.5
            for i in wobble:
                if i+1 or i-1 in mis: ## Mismatches around wobble - Strong penalty
                    score += 1.5
                elif i+1 and i-1 in mis: ## Mismatches on both sides - stronger penalty
                    score += 2
                else:
                    score += 0.5
        else:
...
def mapdd2trans(anIndex):##Creates index on fly and map PARE tags to index  
    ...
    retcode2 = subprocess.call(["/home/bioinfo/tools/bowtie2.1/bowtie2", "-a", "--end-to-end", "-D 1", "-R 1", "-N 0", "-L 20", "-i L,0,1","--score-min L,0,0","--norc","--no-head", "--no-unal", "-t","-p",nthread2, "-f", indexLoc, dd_file,"-S",map_out]) ###Score min sets limimum score to '0'. In ened to end mode that means no mismatch/gap
...
def FileCombine():
    ...
    #targ_fls = [file for file in os.listdir() if re.search(r'all_cdna.00.targ', file)] ## file list using regex
    ...
    os.chdir('../') ## Move out of directory to root
...
def RevMapCoord(ent): ####DOES THIS MODULE WORKS IF USER USES MULTIPLE LIBRARIES - Parallelize
    ...
    if small_win != 0:
        if Local == 'N':
            rev_mapped_entry = ('%s,%s,%s,%s,%s,%s,%s' % (','.join(ent), new_bind_site, new_cleave_site, small_win, large_win, round(small_win/large_win,2), genefunc))
...
def pValueCalculator(target, targetFinderList, proportion):
    ...
    n = sum(x.count(miRNAName) for x in targetFinderList if x[5] <= score)
...
def validatedTargetsFinder(densityDict):
    ...
    for target in targetFinderList:
        ...
        if(gene in densityDict.keys()):
            ...
            cleavageSite = []
            location = 0
            # If there is a match to the 10th and 11th position, we need to
            # break the tie
            if((str(end-9) in currDict) and (str(end-10)
                    in currDict)):
                tenthTarget = currDict[str(end-9)]
                eleventhTarget = currDict[str(end-10)]
                # If the category is lower in the 10th position, use this
                if(tenthTarget[1] < eleventhTarget[0]):
                    cleavageSite = tenthTarget
                    location = end - 9
                # If the category is lower in the 11th position, use this
                elif(tenthTarget[1] > eleventhTarget[1]):
                    cleavageSite = eleventhTarget
                    location = end - 10
                # If the category is the same, break the tie with the highest
                # raw read count
                else:
                    if(tenthTarget[0] > eleventhTarget[0]):
                        cleavageSite = tenthTarget
                        location = end - 9
                    # If the read count is lower in the 11th position, use this
                    elif(tenthTarget[0] < eleventhTarget[0]):
                        cleavageSite = eleventhTarget
                        location = end - 10
                    # If there is still a tie, use the 10th
                    else:
                        cleavageSite = tenthTarget
                        location = end - 9

            # If only the 10th position has a cleavage position, make this
            # the cleavageSite and take its location index.
            elif(str(end-9) in currDict):
                cleavageSite = currDict[str(end-9)]
                location = end - 9

            # If only the 11th position has a cleavage position, make this
            # the cleavageSite and take its location index.
            elif(str(end-10) in currDict):
                cleavageSite = currDict[str(end-10)]
                location = end - 10
        ...
        try:
            ...
            for key in bowtieDict[sequence].keys():
                ...
                for location in bowtieDict[sequence][key]:
                    ...
                    # Append the hits to the hits list 
...
def writeDensityFile(densityDict, mode, allHits, baseCounts, baseCountsOffTagLen,
    outputFile, transcriptomeFilename):
    ...
    twentyPercentile = stats.scoreatpercentile(allHits, 20)
    globalMedian = numpy.median(allHits)
    eightyPercentile = stats.scoreatpercentile(allHits, 80)
    ...
    for gene in sorted(densityDict.keys()):
        if(mode == 0):
            ...
            # Sort all locations in which a tag maps in that gene so they can
            # be listed in order.
            for location in sorted(densityDict[gene].keys()):
                hits = densityDict[gene][location]

                # Calculate category
                if(hits < twentyPercentile):
                    category = '4'
                    categoryCounts[4] += 1
                    densityDict[gene][location] = (hits, 4)
                elif(hits < globalMedian and hits >= twentyPercentile):
                    category = '3'
                    categoryCounts[3] += 1
                    densityDict[gene][location] = (hits, 3)
                elif(hits >= globalMedian and hits <= eightyPercentile):
                    category = '2'
                    categoryCounts[2] += 1
                    densityDict[gene][location] = (hits, 2)
                else:
    ...
    for i in range(len(categoryCounts)):
        # No category in intergenic mode
        if(mode == 1 and i == 1):
            pass
        else:
...
def writeValidatedTargetsFile(header, validatedTargets, outputFile):
    ...
    f.write(header + ',cleavage position,PARE reads,10 nt window abundance,'\
        'PARE reads/window abundance,p-value\n')

    for target in validatedTargets:
        if(float(target[len(target)-1]) < 0.5):
...
def main():
    ...
    if predStep == 'Y':
        ...
        print("Reading the transcriptome and target finder file into memory...")
        start = time.time()
        ...
        print("Step complete. Process took %.3f seconds" % (time.time() - start))
    
        # Store the targetFinder file into memory as a 2D Array.
        print("Storing targetFinder file into memory...")
        start = time.time()
        ...
        print("Finding counts of unambiguous bases...")
        start = time.time()
        ...
        for tagCountFilename in libs:
            ...
            allHits = []
            deGeIndexList = []
            print("Reading %s into memory..." % tagCountFilename)
            start = time.time()
            global tagCountFile
            ...
            print("Creating DeGe Index dictionary in parallel")
            ...
            print("DeeGee Indexing took %.3f seconds" % (time.time() - localStart))
            ...
            print("Finding the validated targets in parallel")
            localStart = time.time()
            ...
            print("All validated targets found in %.3f seconds" % (time.time() - localStart))
    
            # Write the validated targets to a file
            print("Writing the validated targets to a file...")
            start = time.time()
    ...
    if revMapstep == 'Y':
        ...
        for afile in ResFls:
            ...
            revmapRes = '%s_revmapped.csv' % (afile)
            fh_out = open(revmapRes, 'w')
            fh_out.write('%s,BindSite,CleaveSite,SmallWin,LargeWin,Ratio,Func\n' % (header.strip('\n')))
            #
