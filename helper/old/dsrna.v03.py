...
mode        = 0                     ## Mode:0 PHAS with info for both strands Mode:1 miR with info for one strand - - For mode 0, file should have same number of w and c entries in repFiles 
...
def collapseScores(repFile1,repFile2):
    ...
    ### Merge locus-specific data points from different libraries
    mergedscoresD = mergeLibScores(astrandmergeD,bstrandmergeD)
...
def mergeStrandScores(wdict,cdict,afileList):
    '''This function merges strand scores from different strands, by taking avrage of data points, else taking max i.e. datapoint'''
    ...
    if mode == 0:
        print("Both 'w' and 'c' data points exits and will be merged")
        for i in afileList:
            # print("Ent:%s" % (i))
            aname = i[3]
            wlist = wdict[aname]
...
def fileReader(afile):
    ...
    for i in aread:
        ...
        if astrand == 'w':
            wstrandD[aname] = (datap)
            # wstrand.append((aname,astrand,datap))
        elif astrand == 'c':
            # print(datap)
            # print(datap[::-1])
            cstrandD[aname] = (datap[::-1]) ## For crick strand bwtools reverses the positions, setting them back
