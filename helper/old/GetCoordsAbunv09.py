...
####### Settings #######################################
########################################################
mode = 3                                                        ## 0: Get sRNA abundance for specific size and noise 
                                                                ...
                                                                ## 3: Get abundances from clustering results table

tablename   = 'RICE_sbsQIFA_RNAseq_static_based_clustering_100'                ## Tag position summary table required
genomeDB    = 'RICE_MSU7'                                    ## Just to use for cluster name i.e that can be used directly on our website
coords      = 'Table2_21PHAS.coords.txt'
...
chrpos      = '3'                                               ## Excel format
strandpos   = '6'                                               ## Excel format
startpos    = '4'                                               ## End coordinate- excel format
endpos      = '5'                                               ## Start coordinate - excel format

## Small RNA lengths 
mainLen     = [21,21]                                           ## Lengths of sRNAs to calculate abundance and used to compute ratio in mode 0,1,2
noiseLen    = [18,19,21,20,22,23,24]                            ## List of different sizes, used as noise to compute main to noise ratio in mode 0,1,2. In mode 1 both main and nois elist are combined and abundances reported
...
def Coords2Abun(con,coords,db,excludeLibs):
    ...
    if excludeLibs:
        ...
        finalLibs = selectLibs+lastTwo

    else:
        finalLibs = libs+lastTwo
    ...
    for i in fh_in:
        ...
        queryLibs = 'SUM(%s)' % lib_col.replace(",","),SUM(") ## String manipulation to automatically convert library names into query format below - used in all queries
        # print ('These are libraries to be queried',queryLibs)
        # sys.exit()

        ## Prepare quries for input size for both main and noise
        mainSizeQuery = ''
        noiseSizeQuery = ''
        for i in mainLen:
            if mainSizeQuery == '':
        ...
        if useStrand == 'Y':
            cur.execute("select %s from %s.%s where chr_id = %s and strand = '%s' and (%s) and (position between %s and %s)" % (queryLibs,db,tablename,chr_id,strand,mainSizeQuery,str(start2),str(end2)))####
            temp        = cur.fetchall()
            ...
            cur.execute("select %s from %s.%s where chr_id = %s and strand = '%s' and (%s) and (position between %s and %s)" % (queryLibs,db,tablename,chr_id,strand,noiseSizeQuery,str(start2),str(end2)))####
            temp2       = cur.fetchall()
            ...
            cur.execute("select %s from %s.%s where chr_id = %s and (%s) and (position between %s and %s)" % (queryLibs,db,tablename,chr_id,mainSizeQuery,str(start2),str(end2)))####
            temp        = cur.fetchall()
            ###Get alen[1] i.e 24,22 or whatever mentioned in settings
            #cur.execute("select sum(lib_2435),sum(lib_2436),sum(lib_2437),sum(lib_2438),sum(lib_2439),sum(lib_2440),sum(lib_2441),sum(lib_2495),sum(lib_2496),sum(lib_2497),sum(lib_2498),sum(lib_2499),sum(lib_2500),sum(lib_2501),sum(lib_2502),sum(lib_2503),sum(lib_2504),sum(lib_2505),sum(lib_2506),sum(lib_2507),sum(lib_2508),sum(lib_2509),sum(lib_2510),sum(lib_2511),sum(lib_2512),sum(lib_2513),sum(lib_2514),sum(lib_2515),sum(lib_2516),sum(lib_2517),sum(lib_2518),sum(lib_2519),sum(lib_2520),sum(lib_2521),sum(lib_2522),sum(lib_2596),sum(lib_2597),sum(lib_2598),sum(lib_2599),sum(lib_2600),sum(lib_2601),sum(lib_2602),sum(lib_2523),sum(lib_2524),sum(lib_2525),sum(lib_2526),sum(lib_2527),sum(lib_2528),sum(lib_1588),sum(lib_1589),norm_sum,max_norm from %s.%s where chr_id = %s and len = %s and (position between %s and %s)" % (db,tablename,chr_id, alen[1],str(start2),str(end2)))####
            cur.execute("select %s from %s.%s where chr_id = %s and (%s) and (position between %s and %s)" % (queryLibs,db,tablename,chr_id,noiseSizeQuery,str(start2),str(end2)))####
            temp2       = cur.fetchall()
...
def compCoordAbun(con,coords,db,excludeLibs):
    ...
    if excludeLibs:
        print("\n\nLibs specifed in excludeLibs %s will be skipped" % (excludeLibs))
        selectLibs = [] ## Columns excluding the unwanted libraries
        excludeLibs_s = [str(i) for i in excludeLibs] ## Converting all entries in exclude list to string fro matching below
    ...
    for i in fh_in:
        ...
        # resMatrix = [0]*len(allLen) ## To store all results, required for result format 1
        # resMatrix2 = [0]*len(allLen) ## To store just Norm sum for all sizes required for result format 2
        ...
        if useStrand == 'Y':
            # resPos = 0 ## To record results in correct positions
            for size in allLen:
                # print("Size of sRNAs being collected:%s" % (str(size)))
                cur.execute("select %s from %s.%s where chr_id = %s and strand = '%s' and (len = %s) and (position between %s and %s)" % (queryLibs,db,tablename,chr_id,strand,size,str(start2),str(end2)))####
                temp = cur.fetchall()
                abundances = ",".join(str(x) for x in temp[0]) ## in comma separated format
                mainNormSum = temp[0][-2]
            ...
            for size in allLen:
                # print("Size of sRNAs being collected:%s" % (str(size)))
                #cur.execute("select sum(lib_2435),sum(lib_2436),sum(lib_2437),sum(lib_2438),sum(lib_2439),sum(lib_2440),sum(lib_2441),sum(lib_2495),sum(lib_2496),sum(lib_2497),sum(lib_2498),sum(lib_2499),sum(lib_2500),sum(lib_2501),sum(lib_2502),sum(lib_2503),sum(lib_2504),sum(lib_2505),sum(lib_2506),sum(lib_2507),sum(lib_2508),sum(lib_2509),sum(lib_2510),sum(lib_2511),sum(lib_2512),sum(lib_2513),sum(lib_2514),sum(lib_2515),sum(lib_2516),sum(lib_2517),sum(lib_2518),sum(lib_2519),sum(lib_2520),sum(lib_2521),sum(lib_2522),sum(lib_2596),sum(lib_2597),sum(lib_2598),sum(lib_2599),sum(lib_2600),sum(lib_2601),sum(lib_2602),sum(lib_2523),sum(lib_2524),sum(lib_2525),sum(lib_2526),sum(lib_2527),sum(lib_2528),sum(lib_1588),sum(lib_1589),norm_sum,max_norm from %s.%s where chr_id = %s and len = %s and (position between %s and %s)" % (db,tablename,chr_id, alen[0],str(start2),str(end2)))####
                cur.execute("select %s from %s.%s where chr_id = %s and (len = %s) and (position between %s and %s)" % (queryLibs,db,tablename,chr_id,size,str(start2),str(end2)))####
                temp = cur.fetchall()
                ###Get alen[1] i.e 24,22 or whatever mentioned in settings
                #cur.execute("select sum(lib_2435),sum(lib_2436),sum(lib_2437),sum(lib_2438),sum(lib_2439),sum(lib_2440),sum(lib_2441),sum(lib_2495),sum(lib_2496),sum(lib_2497),sum(lib_2498),sum(lib_2499),sum(lib_2500),sum(lib_2501),sum(lib_2502),sum(lib_2503),sum(lib_2504),sum(lib_2505),sum(lib_2506),sum(lib_2507),sum(lib_2508),sum(lib_2509),sum(lib_2510),sum(lib_2511),sum(lib_2512),sum(lib_2513),sum(lib_2514),sum(lib_2515),sum(lib_2516),sum(lib_2517),sum(lib_2518),sum(lib_2519),sum(lib_2520),sum(lib_2521),sum(lib_2522),sum(lib_2596),sum(lib_2597),sum(lib_2598),sum(lib_2599),sum(lib_2600),sum(lib_2601),sum(lib_2602),sum(lib_2523),sum(lib_2524),sum(lib_2525),sum(lib_2526),sum(lib_2527),sum(lib_2528),sum(lib_1588),sum(lib_1589),norm_sum,max_norm from %s.%s where chr_id = %s and len = %s and (position between %s and %s)" % (db,tablename,chr_id, alen[1],str(start2),str(end2)))####
                abundances = ",".join(str(x) for x in temp[0]) ## in comma separated format
    ...
    for i in fh_in:
        ...
        if useStrand == 'Y':
            for size in allLen:
                ...
                cur.execute("select count(*) from %s.%s where chr_id = %s and strand = '%s' and (len = %s) and (position between %s and %s) and (%s)" % (db,tablename,chr_id,strand,size,str(start2),str(end2),querycon1))
                temp = cur.fetchall()
                sizeCount =  temp[0][0]
            ...
            for size in allLen:
                # print("Size of sRNAs being collected:%s" % (str(size)))
                cur.execute("select count(*) from %s.%s where chr_id = %s and (len = %s) and (position between %s and %s)" % (db,tablename,chr_id,size,str(start2),str(end2)))
                temp = cur.fetchall()
                sizeCount =  temp[0][0]
                resList.append(sizeCount)
...
def cluster2abun(con,coords,db,excludeLibs):
    '''
    gets the abundances from the clustering file
    '''
    ...
    if header == 'Y':
        fh_in.readline()
    outfile     = coords.rpartition('.')[0]+'clustabun.csv'
    ...
    libs        = columns[9:-2] ## norm_sum and max_norm are last two columns
    lastTwo     = columns[-2:]  ## norm_sum and max_norm
    ...
    lib_col =",".join(str(x) for x in finalLibs)### Manually mentioned starting lib column - should work on all tag position summary tables
    print(lib_col)
    
    #### Write file header
    fh_out.write('Loci,Cluster,%s,NormSum\n' % (lib_col))
    ...
    for i in fh_in:
        ...
        if useStrand == 'Y':
            strand  = str(coord[int(strandpos)-1]).strip()
        print ('chr_id:%s | start:%s | end:%s | length: %s bp' % (chr_id,str(start2),int(end2),length))
        cur = con.cursor()
        ...
        if useStrand == 'Y':
            cur.execute("select %s from %s.%s where chr_id = %s and strand = '%s' and (end >= %s and start <= %s)" % (queryLibs,db,tablename,chr_id,strand,str(start2),str(end2)))####
            temp        = cur.fetchall()
            lociname    = '%s_%s_%s_%s' % (chr_id,strand,start,end)
            clusterName = '%s.%s.%s.%s' % (genomeDB,chr_id,start,end)
            ...
            #cur.execute("select sum(lib_2435),sum(lib_2436),sum(lib_2437),sum(lib_2438),sum(lib_2439),sum(lib_2440),sum(lib_2441),sum(lib_2495),sum(lib_2496),sum(lib_2497),sum(lib_2498),sum(lib_2499),sum(lib_2500),sum(lib_2501),sum(lib_2502),sum(lib_2503),sum(lib_2504),sum(lib_2505),sum(lib_2506),sum(lib_2507),sum(lib_2508),sum(lib_2509),sum(lib_2510),sum(lib_2511),sum(lib_2512),sum(lib_2513),sum(lib_2514),sum(lib_2515),sum(lib_2516),sum(lib_2517),sum(lib_2518),sum(lib_2519),sum(lib_2520),sum(lib_2521),sum(lib_2522),sum(lib_2596),sum(lib_2597),sum(lib_2598),sum(lib_2599),sum(lib_2600),sum(lib_2601),sum(lib_2602),sum(lib_2523),sum(lib_2524),sum(lib_2525),sum(lib_2526),sum(lib_2527),sum(lib_2528),sum(lib_1588),sum(lib_1589),norm_sum,max_norm from %s.%s where chr_id = %s and len = %s and (position between %s and %s)" % (db,tablename,chr_id, alen[0],str(start2),str(end2)))####
            cur.execute("select %s from %s.%s where chr_id = %s and (end >= %s and start <= %s)" % (queryLibs,db,tablename,chr_id,str(start2),str(end2)))####
            temp        = cur.fetchall()
            lociname    = '%s_%s_%s' % (chr_id,start,end)
            clusterName = '%s.%s.%s.%s' % (genomeDB,chr_id,start,end)
        ...
        mainNormSum     = temp[0][-2]
        abundances      = ",".join(str(x) for x in temp[0]) ## in comma separated format
        print('Norm sum for main:%s' % (mainNormSum))
        
        fh_out.write('%s,%s,%s,%s\n' % (lociname,clusterName,abundances,str(mainNormSum)))
...
def main(db,coords):
    ...
    if mode == 0:
        ...
        resFile1 = cluster2abun(con,coords,db,excludeLibs)
    else:
...
if __name__ == '__main__':
    ...
    print('**Script Finished Succesfuly')
    sys.exit()
...
## v08 -> v09
## Added functionality (mode-3) to compute co-ordinate abundances from clustering table
