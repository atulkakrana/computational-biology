...
startpos =  '4' #### coordinate strat- excel format
endpos = '5' ## coordinate end - excel format
#alen = [21,24] ### Length of sRNAs to calculate abundance for
mainLen = 24
noiseLen = [21,23] ## A list with range of size i.e. 22 to 24-  choose one if required i.e [24]
...
def Coords2Abun(con,coords,db):
    ...
    for i in fh_in:
        ...
        if useStrand == 'Y':
            cur.execute("select %s from %s.%s where chr_id = %s and strand = '%s' and len = %s and (position between %s and %s)" % (queryLibs,db,tablename,chr_id, strand, mainLen,str(start2),str(end2)))####
            temp = cur.fetchall()
            ...
            cur.execute("select %s from %s.%s where chr_id = %s and len = %s and (position between %s and %s)" % (queryLibs,db,tablename,chr_id, mainLen,str(start2),str(end2)))####
            temp = cur.fetchall()
            ###Get alen[1] i.e 24,22 or whatever mentioned in settings
