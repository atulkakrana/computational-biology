...
SpecStrand = 'N' 						## If strand is available and sequence needs to be from specific strand, If 'N' than you dont care about strand and want from both
...
def Coords2FASTA(con,coords,genomedb,flankType,buff):
    ...
    for i in fh_in:
        ...
        if SpecStrand == 'Y': ##Strand info available - Like if coordinates are for gene
            ...
            fh_out.write('>%s_w_%s:%s:%s\n%s\n' % (coord[0],chr_id,start2,end,temp[0][0]))
            fh_out.write('>%s_c_%s:%s:%s\n%s\n' % (coord[0],chr_id,start2,end,temp[1][0][::-1]))### Reversed to give correct orientation
