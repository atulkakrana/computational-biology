...
phasFile        = "Final_21PHAS_Loci_ALL_v9.bed"                            ## Phased file which needs to be checke for overlap
...
exonOverlapPerc     = 0                                                      ## Minimum percentage of PHAS covered by exon. Keep 0 for finding overlapping genes and 0.20 or above for finding precursors
exonsOverlapCutoff  = 0                                                      ## Values of minimum overlap in nts. This is required if PHAS overlap is checked for precursors and not overlap with any regio of genes. FOr precursors one will expect an overlap of 4 phases or more
featureFile     = "AsparagusCHR_V1.1.evmRun2.withintrons.mod.gtf"                                   ## Feature file in GTF format, with whome overlap needs to be checked
featureFile2    = "All_A_B_C_sizes_polished_high_qv_consensus_isoforms.mod.collapse.gtf"                                             ## Second feature file if overlap needs to be checked with two lists, but independently
mode            = 3                                                         ## 1: PASA feature (GTF) file | 2: Trinity | 3: Rocket feature (GTF) file or generic gtf file with missing "transcript" entry | 4: Both rocket and trinity feature files | 5: PacBio GTF

phasedFeature   = "Final_21PHAS_Loci_0.0005_ALL.csv"               ## [Optional, give an empty file] Phasredundant file of transcripts from feature file, to label overapping transcripts as true phased. Provide concataned file if both rocket and trinity GTF file used in mode3

strandbiasStep  = 0                                                         ## 0: No | 1: yes
mainLen         = [21,21]                                                   ## For stand Ratio
noiseLen        = [24,23,22]                                                ## For strand ratio
includeLibs     = ["Asp_0_5_ant_bud"]        ## Libraries to compute strand ratio, pre-meiotic:  | Meiotic:"ant1_07mm_r1","Asp_1_ant_bud"
...
def main():
    ...
    if aStep == 1:
        ...
        elif mode == 2 or mode == 3 or mode == 5: ## Trinity or Rocket GTF for overlapping transcipts
