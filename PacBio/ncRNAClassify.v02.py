...
## USER SETTINGS ##################################################################
summary         = "summary.v3.txt"       ## Should be tab seprated file, because overlap columns have comma seprated values                        
...
def classify(summary,cpatThres):
    ...
    for i in summRead:
        ...
        ## matchScore >= 3 - Matches well to protein coding gene
        if matScore == 'NA':
            ...
            ## These must be coding, except short ORF
            if orf > ORFThres:
                status = 'co'
                resList.append((trans,status))
                acount+=1
            else:
                ## Small ORF - Maybe ncRNA that matched annot - Needs testing
                if cpcScore >= 1:
                    status = 'co'
                    resList.append((trans,status))
                    acount+=1
        ...
        elif int(matScore) < 3 and int(matScore) >= 0:
