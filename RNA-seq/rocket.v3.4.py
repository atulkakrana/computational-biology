...
## assembly process - It can be used for assembly and abudnance quantification
...
## ROCKET ####################################
genoFile        = "./rice.msu7.chr.all.mod.fa"
gtfFile         = "Osativa_323_v7.0.gene_exons.gtf" 
genoIndex       = "./index/rice.msu7.chr.all"               ## If index is not in $ALLDATA i.e. local analysis, then specify bowtie1 index here for pre-processing. For Seq-analysis a Bowtie2 index  will be made using 'indexBuilderStep'
sampleInfo      = "sampleInfoPrivateSE.csv"                  ## [mandatory] Tab delimted file with three mandatory columns - num (sample numbers), id (filename,library id), rep (same number if replicates
...
userMaxTagLen   = 100                   ## [server] Used if 'hardMinTagLen' is ON
...
maxLen          = 100                   ## [mandatory] Max length of the tag allowed. Based on maxLen mismatches are allowed for mapping
unpairDel       = 1                     ## [Only for paired end analysis] 0: Retain unpaired read files after trimming 1: Delete these files

numProc         = 46                    ## [developer]  Coarse grain PP [0: Maximize parallel processing | [1-64]: Number of Cores]
nthread         = 8                    ## [developer]  Fine grain PP
...
def cuffLinks(aninput):
    ...
    retcode = subprocess.call([cufflinks,"-p", nthread,"-o", outDir, inFile])
