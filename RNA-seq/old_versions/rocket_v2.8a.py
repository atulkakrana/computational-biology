...
referenceGTF    = 'F'                   ## [optional] T: True - use reference gtf file for merging assembling and annotation | F: Do not use GTF file and report transcripts based on transcriptome
...
userMaxTagLen   = 145                    ## [server] Used if 'hardMinTagLen' is ON
...
maxLen          = 145                    ## [mandatory] Max length of the tag allowed. Based on maxLen mismatches are allowed for mapping
...
## TOOL/FILE PATH ###################################
adapterFileSE   = '/data1/homes/kakrana/tools/Trimmomatic-0.32/adapters/TruSeq3-SE.fa'      ## [mandatory] Sequence adapter file in FASTA format - Trimmomatic has files for different kits - Copy from there
adapterFilePE   = '/data1/homes/kakrana/tools/Trimmomatic-0.32/adapters/TruSeq3-SE.fa'      ## You can try merged file with SE and PE adapter too, it gave best results to Atul, because adapters are sometimes not in pairs
...
def trimLibs(aninput):
    ...
    if Local == 1: 
        ...
        if seqType == 0:
            ...
            retcode = subprocess.call(["java", "-jar", toolPath, "SE", "-phred33", "-trimlog",trimLog,"-threads", nthread, infile, trimmedFile, "ILLUMINACLIP:%s:2:30:10" % (adapterFile), "LEADING:3", "TRAILING:3", "SLIDINGWINDOW:4:10", "MINLEN:%s" % (minTagLen)])
        ...
        ## Paired End ##################
            ...
            retcode = subprocess.call(["java", "-jar", toolPath, "PE", "-phred33", "-trimlog",trimLog,"-threads", nthread, infile1, infile2, trimmedFileP1,trimmedFileU1,trimmedFileP2,trimmedFileU2, "ILLUMINACLIP:%s:2:30:10" % (adapterFile), "LEADING:3", "TRAILING:3", "SLIDINGWINDOW:4:10", "MINLEN:%s" % (minTagLen)])
            if retcode == 0:## The bowtie mapping exit with status 0, all is well
                    print('\n****Trimming for %s complete****' % (infile) )
    ...
    ## SERVER BASED ##########################
        ...
        ## Paired End ##################
            ...
            retcode = subprocess.call(["java", "-jar", toolPath, "PE", "-phred33", "-trimlog",trimLog,"-threads", nthread, infile1, infile2, trimmedFileP1,trimmedFileU1,trimmedFileP2,trimmedFileU2, "ILLUMINACLIP:./%s_adapter.fa:2:30:10" % (lib), "LEADING:3", "TRAILING:3", "SLIDINGWINDOW:4:10", "MINLEN:%s" % (minTagLen)])
            if retcode == 0:## The bowtie mapping exit with status 0, all is well
                    print('\n****Trimming for %s complete****' % (infile) )
            else:
                print('Something wrong happened while chopping library: %s - - Debug for reason' % (lib))
...
def splicedMapper(rawInputs,genoIndex,gtfFile):
    ...
    for aninput in rawInputs:
        ...
        if referenceGTF == 'T':
            ...
            if seqType == 0:
                retcode = subprocess.call([tophat,"-p", nproc2, "-o", mapFile, genoIndex, inFile])
            else:
                retcode = subprocess.call([tophat,"-p", nproc2, "-o", mapFile, genoIndex, inFile1, inFile2])
