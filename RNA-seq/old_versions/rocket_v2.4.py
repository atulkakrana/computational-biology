...
genomeDB = 'GRAPE_IGGP12Xv1_genome'      ## For Bowtie index path used for mapping for graphs
## ROCKET ####################################
genoFile = '/alldata/Genomic/Grape/GENOSCOPE/v1/GenomicSEQ/XXXXXX'
...
userMinTagLen = 20                     ## Used if 'hardMinTagLen' is ON
...
## SEQ_ANALYSIS - REQUIRED STEPS [Value: 0/1] ##############
...
## ADVANCED SETTINGS #######################
Local = 1                               ## 1: Local 0: Remote Run local Analysis: Requires - maxLen,minLen,maxReadLen and adpaters.fa and libraries
maxLen = 22                             ## Max length of the tag allowed - BASED on maxLen mismatches are allowed for mapping
minLen = 18                             ## Min length of tag allowed
adapterFile = 'TruSeq-SE.fa'                        ## Sequence adapter file in FASTA format - Trimmomatic has files for different kits - Copy from there
genoIndex = '/alldata/Genomic/Grape/GENOSCOPE/v1/BowtieGenomicIndexes/GRAPE_IGGP12Xv1_genome'               ## IF index is not in $ALLDATA i.e. external analysis
...
import datetime,operator,subprocess,multiprocessing,matplotlib
