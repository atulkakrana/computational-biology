...
## ROCKET ####################################
genoFile = '/home/kakrana/99.genomes/Oryza_sativa.IRGSP-1.0.24.dna_sm.genome/Oryza_sativa.IRGSP-1.0.24.dna_sm.genome.fa'
referenceGTF = 'F' ## T: True - use reference gtf file for merging assembling and annotation | F: Do not use GTF file and report transcripts based on transcriptome
gffFile = '/home/kakrana/99.genomes/Oryza_sativa.IRGSP-1.0.24.dna_sm.genome/Oryza_sativa.IRGSP-1.0.24.gtf' ## http://plants.ensembl.org/info/website/ftp/index.html
libType = 0 ## From cuffNorm manual 1) 0 = fr-unstranded 2) 1 = fr-firststrand 3) 2 = fr-secondstrand

# ATUL - THESE ARE THE MAIN LIBS AFTER SPLICE MAPPER STEP - TURN THESE ON FOR REST OF ANALYSIS
# libs = [3783,3791,3771,3780,3777,3792,3778,3785,3779,3794,3967,3979,3968,3980,4381,4386] ## Hand picked for maximum variety (less replicates), sterile have been removed
# reps = [1,1,2,2,3,3,5,5,7,7,10,10,11,11,21,22]

hardMinTagLen = 'Y'
userMinTagLen = 40

libs = [4382,4383,4384,4385,3784,3793,3786]
reps = [21,21,22,22,3,5,7]

# libs_bak = [3775,3787,3784,3793,3786,3772,3781,3788,3773,3782,3789,3744,3795,3790,3966,3978,3972,3984,3973,3985,3974,3986,4382,4383,4384,4385]
# reps_bak = [1,2,3,5,7,4,4,4,6,6,6,8,8,8,9,9,12,12,13,13,14,14,21,21,22,22]
...
## ADVANCED SETTINGS ###########################
Local = 'N' ## Run local Analysis: Depends upon maxLen,minLen, maxReadLen, adpaters.fa and files locally
#genoIndex = 'xyz'
...
def mapper(con,rawInputs,mode):
    ...
    for aninput in rawInputs:
        ...
        retcode = subprocess.call(["bowtie","-f","-n 0","-p", nproc2,"-t" ,genoIndexPrePro, fastaFile, mapFile])
...
def main(libs):
    ...
    rawInputs = [(i[0],'trimmed.fastq',i[2],i[7]) for i in register]
    ...
    rawInputs = [(i[0],'chopped.trimmed.fastq',i[2],i[3]) for i in register]
    if QCheckStep == 1:
    ...
    if cleanupStep == 1:
        ...
        garbage = [afile for afile in os.listdir('./') if file.endswith (('trim.log','processed.fa','.zip'))] ## Excluded 'chopped.trimmed.fastq' as used by RNA Runner
