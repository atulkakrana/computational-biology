...
genoIndex       = "/home/kakrana/8.Collab/4.Kak/99.genome/GenomeM.toplevel.index.bowtie1/GenomeM.toplevel.index"                ## If index is not in $ALLDATA i.e. local analysis, then specify bowtie1 index here for pre-processing. For Seq-analysis a Bowtie2 index  will be made using 'indexBuilderStep'
...
tophat          = '/home/kakrana/tools/tophat-2.1.1.Linux_x86_64/tophat2'           ## [mandatory]
...
def splicedMapper(rawInputs,genoIndex,gtfFile):
    ...
    for aninput in rawInputs:
        ...
        if seqType == 0:
            inFile = '%s.%s' % (lib,ext)
            print ('Mapping %s to genome for RNA-Seq analysis' % (inFile))
...
def cuffLinks(aninput):
    ...
    retcode = subprocess.call([cufflinks,"--max-bundle-frags", maxfrags2,"-p", nthread,"-o", outDir, inFile])
