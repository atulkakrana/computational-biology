...
def trimLibs(aninput):
    ...
    if Local == 1: 
        ...
        if seqType == 0:
            ...
            retcode = subprocess.call(["java", "-jar", toolPath, "SE", "-phred33","-threads", nthread, infile, trimmedFile, "ILLUMINACLIP:%s:2:30:10" % (adapterFile), "LEADING:3", "TRAILING:3", "SLIDINGWINDOW:4:10", "MINLEN:%s > %s 2>&1" % (minTagLen,trimSumm)])
        ...
        ## Paired End ##################
            ...
            retcode = subprocess.call(["java", "-jar", toolPath, "PE", "-phred33", "-threads", nthread, infile1, infile2, trimmedFileP1,trimmedFileU1,trimmedFileP2,trimmedFileU2, "ILLUMINACLIP:%s:2:30:10:8:TRUE" % (adapterFile), "LEADING:3", "TRAILING:3", "SLIDINGWINDOW:4:15", "MINLEN:%s > %s 2>&1" % (minTagLen,trimSumm)])
            if retcode == 0:## The bowtie mapping exit with status 0, all is well
                    print('\n****Trimming for %s complete****' % (infile) )
