...
transFile = 'genes.attr_table' ## Required for extracting transcript
genoFile = '/home/kakrana/99.genomes/Oryza_sativa.IRGSP-1.0.24.dna_sm.genome/Oryza_sativa.IRGSP-1.0.24.dna_sm.genome.fa'
mode = 2 ## 1: Extract CDS [use merged GTF] | 2: Extract Gene transcript [use genes attribute file] | 3: Extract Isoforms [use isofor attribute file] 4: Extract Promoters [use gene attribute file]
			...
			if transcript_id == ent2[5]:
				...
				## This is last exon of multi exon gene
					...
					if strand == 'w':
						...
						seqList.append((gene_id,transcript_id,featureSeq[::-1].translate(str.maketrans("TAGC","ATGC"))))
					else:
						## Strand is uknown and must be '.' in file - So record both
						print("GeneID:%s - Unknown strand" % (gene_id))
						seqList.append((gene_id,transcript_id,featureSeq))
						seqList.append((gene_id+'_rc',transcript_id+'_rc',featureSeq[::-1].translate(str.maketrans("TAGC","ATGC"))))
