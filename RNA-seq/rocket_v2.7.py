...
Local           = 0                               ## 1: Local 0: Remote | NOTE: Local Analysis: Requires - maxLen,minLen,maxReadLen and adpaters.fa and libraries
                                                  ## Requires SampleInfo file with sampleNum, Filename/Lib code, reps, group
genomeDB        = 'ASPARAGUS_UGA2_genome'         ## [Server mode]For Bowtie index path used for mapping for graphs
## ROCKET ####################################
genoFile        = '/alldata/Genomic/Asparagus/UGA/v2/GenomicSEQ/Asparagus.V2.0.genome.stable_modifed.fa' 
gtfFile         = '/home/kakrana/99.genomes/asparagus/v2/AsOf.V2.evm1.gtf'  
genoIndex       = '/alldata/Genomic/Asparagus/UGA/v2/BowtieGenomicIndexes/ASPARAGUS_UGA2_genome'               ## If index is not in $ALLDATA i.e. local analysis, then specify bowtie1 index here for pre-processing. For Seq-analysis a Bowtie2 index  will be made using 'indexBuilderStep'
sampleInfo      = "sampleInfo.txt"       ## [mandatory] Tab delimted file with three mandatory columns - num (sample numbers), id (filename,library id), rep (same number if replicates)
...
userMaxTagLen   = 150                    ## [server] Used if 'hardMinTagLen' is ON
...
## ADVANCED SETTINGS #######################
maxLen          = 150                    ## [mandatory] Max length of the tag allowed. Based on maxLen mismatches are allowed for mapping
...
numProc         = 0                     ## [developer]  Coarse grain PP [0: Maximize parallel processing | [1-64]: Number of Cores]
...
dataServer      = 'raichu.dbi.udel.edu' ## [server]
...
adpaterFilePE   = '/data1/homes/kakrana/tools/Trimmomatic-0.32/adapters/TruSeq-PE.fa'
...
def trimLibs(aninput):
    ...
    ## SERVER BASED ##########################
        ...
        ## Paired End ##################
            ...
            retcode = subprocess.call(["java", "-jar", toolPath, "PE", "-phred33", "-threads", nthread, infile1, infile2, trimmedFileP1,trimmedFileU1,trimmedFileP2,trimmedFileU2, "ILLUMINACLIP:./%s_adapter.fa:2:30:10" % (lib), "LEADING:3", "TRAILING:3", "SLIDINGWINDOW:4:15", "MINLEN:%s" % (minTagLen)])
            if retcode == 0:## The bowtie mapping exit with status 0, all is well
                    print('\n****Trimming for %s complete****' % (infile) )
            else:
                print('Something wrong happened while chopping library: %s - - Debug for reason' % (lib))
...
def splicedMapper(rawInputs,genoIndex,gtfFile):
    ...
    for aninput in rawInputs:
        ...
        ## Bowtie 1 - So as to be compatible with current indexes
        if seqType == 0:
            retcode = subprocess.call([tophat,"-p", nproc2,"-G", gtfFile, "-o", mapFile, genoIndex, inFile])
        else:
            retcode = subprocess.call([tophat,"-p", nproc2,"-G", gtfFile, "-o", mapFile, genoIndex, inFile1, inFile2])
    ...
    if QCheckStep == 1:
        ...
        if Local == 1: # Provide local file name
            if seqType == 0: ## Single-end
                rawInputs = [(i[0],i[1],i[2],'%s.fastq' % (i[0])) for i in register] ## ## Library/Filename, nthread, extension
            elif seqType == 1: ## Paired end
                inputsR = [(i[0],i[1],i[2],'%s_1.fastq' % (i[0])) for i in register] ## ## Library/Filename, nthread, extension
                inputsL = [(i[0],i[1],i[2],'%s_2.fastq' % (i[0])) for i in register] ## ## Library/Filename, nthread, extension
            ...
            if seqType == 0: ## Single End
                rawInputs = [(i[0],i[1],i[2],i[3]) for i in register] ## ## Library/Filename, nthread, extension, and input file path
            elif seqType == 1: ## PairedEnd
                inputsR = [(i[0],i[1],i[2],'%s_1.fastq' % (i[3].rpartition("_")[0])) for i in register] ## ## Library/Filename, nthread, extension, filepath
                inputsL = [(i[0],i[1],i[2],'%s_2.fastq' % (i[3].rpartition("_")[0])) for i in register] ## ## Library/Filename, nthread, extension, filepath
                rawInputs = inputsL+inputsR
            else:
                print("Please input seqType - SE or PE")
...
## v2.6 -> v2.7
## Modofied sample info file reading, first column in lib_id/lib_num , filename/lib_code, rep, group - In case of server mode lib_id is used and in case of local mode filename is used
