...
import re,sys,os,subprocess,csv,glob,time,random,math
...
testProbes = 'test.list' ## tab seprated file with probe ID as first column and gene symbol as second column


annoFile = 'Mouse430_2.na34.annot.csv' ### Clean Header lines before using here
resFile = 'AF_tooth_GCRMA_SEPv1_WBforLens_Rank.csv'
resSep = '\t'
...
#cols=[49,10] ## T-stats -  Col number and Number of tracks (total 10 tracks in current file)
...
def tableMaker(trackTableName,annoDict,resList,tableCols):
    '''
...
def relativeRankGenerator(probe,trackTableName,chrLimit):
    '''
    ...
    try:
        ...
        print ('\nCoordinates found for:%s - %s' % (probe,','.join(str(x) for x in probeCoord)))
    except KeyError:
        print ('''\nCoordinates not found for:%s
        #Random co-ordinates will be selected for relativeRank''' % (probe))
        ## Randomly select a co-ordinate i.e select a probeID
        randomProbe = randomSelect(annoProbeSET,1) ## Use annoProbeSet - as its bound to have co-ordinates
        randomProbe_strp = randomProbe[0].replace("'",'')
        probeCoord = annoDict[randomProbe_strp]
        print ('Randomly selected co-ordinate:%s\n\n' % (','.join(str(x) for x in probeCoord)))
    ...
    #print('Probe:%s | Chr:%s | Start:%s | End:%s' % (probe,chromo,start,end))
    ...
    if end >= chrLimit[chromo]:
        ...
        if chromo == min(list(chrLimit.keys())): ## That means you cant go to earlier chromosme, move to last chromosome i.e last chromosme
            carryChromo = min(list(chrLimit.keys()))
        else:          
            carryChromo = chromo-1
...
def pvalCalcAll(col):
    ...
    if rankStat == 'FC':
        ...
        if head[col].split('.')[0] == 'FC':  ## Confirms FC as column               
            ...
            #trackTable = tableMaker(conn,trackTableName,annoDict,resList,tableCols)
            trackTableName = tableMaker(trackTableName,annoDict,resList,tableCols) ##Makes the table and resturn the name itself - Does't matter
            chrLimit = chrLimits(trackTableName)
            ...
            for ent in resList: ##
                ...
                geneSym = ent[1]
                mainRank = relativeRankGenerator(mainProbe,trackTableName,chrLimit)
                ...
                for randProbe in randomProbes:
                    randomRank = relativeRankGenerator(randProbe,trackTableName,chrLimit)
...
def pValCalcTest(col,testProbes):
    ...
    if rankStat == 'FC':
        ...
        if head[col].split('.')[0] == 'FC':  ## Confirms FC as column               
            ...
            trackTableName = 'track_%s' % (col)
            #trackTable = tableMaker(conn,trackTableName,annoDict,resList,tableCols)
            trackTableName = tableMaker(trackTableName,annoDict,resList,tableCols) ##Makes the table and return the name itself - Does't matter
            chrLimit = chrLimits(trackTableName)
            
            ## Itnerate every gene of current track - Do all processing
            for ent in testProbes: ##
                print(ent)
                ...
                if mainProbe not in resDict:
                    trackPvals.append(round(0.999999)) ## main probe is missing in this microaary experiment - so not enriched
                    print('\n\n main probe misisng \n\n')
...
def trackManager(resFile,cols,nproc):
...
##----------------------------------------------------------------------------------------------------
    ...
    if allProbes == 'N':
        allTrackPvals = []
        for i in trackList:
            trackPvals = pValCalcTest(i,testProbeList)
    ...
    #allTrackPvals = PPResults(pvalCalc,trackList) ## How to sort results on basis of tracks - Perhaps generate results in form of dictionary entry with track name as key
    #print(allTrackPvals)
    
    ##Create one main dict with tracks cols as key and track results as value - Will be used for resorting results trackwise before mergibng
    allTrackPvalsDict = {}
    for i in allTrackPvals:
        allTrackPvalsDict.update(i)
    
    print(allTrackPvalsDict)
    
    ###Sort the results track wise
...
##----------------------------------------------------------------------------------------------------
    ...
    ##Open Result File
    resultFile = 'Testpvals_%s.csv' % (datetime.datetime.now().strftime("%m_%d_%H_%M"))
...
def main():
    ...
    #pvalCalc(resFile,cols,nproc)
    resultFile = trackManager(resFile,cols,nproc)
