...
##V.1B gives hits normalized abundances instead of sum of norm_abundances
...
#    for len in range (20,22):
    ...
    #    print(int(clust[1])+int(clust[2]))
        ...
        mer_abun = cur.fetchall()
        return mer_abun ## for specified 'mer' in the particular cluster it gives tag, norm_sum, hits
    #    print (mircount[0])
...
fh_out = open('clust_hit_norm_abun' , 'w')
fh_out.write('Cluster    \tLen\tHit_Norm_Abun\tTags\tLen\tHit_Norm_Abun\tTags\n')
...
for clust in clust_list:
    ...
    for mer in range (21,23):## For different size of miRNAs                 
        ...
        for ent in mer_abun: ## Loop to add up abundances for all the tags and later divide by total tags (num_tags) for average abundance
            tag_abun = ent[1] ## norm_sum for each tag
            hits_norm_abun = ent[1]/ent[2]## Here the normalized abundance is divided by number of hits for it in genome
            abun_sum += hits_norm_abun ## Here we sum up the hit normalized abundances for all the tags of particular length for a cluster
...
print ('Total number of clusters analyzed in analysis: %s' % (clust_num))
