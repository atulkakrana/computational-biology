...
def Extractor(master,query):
    ...
    for i in query_read:
        i = i.strip('\n')
        #print(i)
        key = i.split(args.delimiter2)[int(args.position2)].upper()###Key is the value on which matching will be done, upper case
        print(key)
        query_set.add(key)
    ...
    for key in query_set:
        ...
        for entry in master_read:
            ...
            if key==entry_splt[int(args.position)].upper():#Both key and query should be at same case
...
##June 2nd v02 -> v03
#Both key and query terms are forced to same case before matching
