...
def PHASBatch(con,libs,geno,geno_index):
    ...
    for lib in libs:
        ...
        if geno == 'Y':###Uses Whole genome as input
            retcode = subprocess.call(["/usr/local/bin/perl", "/data2/homes/kakrana/svn/users/pingchuan/phasiRNA_prediction_pipeline.ver.genome.pl", "-i", pro_file, "-f", "t", "-d", geno_index, "-px", out_file, "-rl", rl, "-cpu", nproc2])
            
        else: ### Uses FASTA file of genes as input         
            #pipe =subprocess.Popen(["/usr/local/bin/perl5.18", "-v"])
            retcode = subprocess.call(["/usr/local/bin/perl", "/data2/homes/kakrana/svn/users/pingchuan/phasiRNA_prediction_pipeline.ver.MUL.pl", "-i", pro_file, "-f", "t", "-d", geno_index, "-px", out_file, "-rl", rl, "-cpu", nproc2])
...
def main(server):
    ...
    libs = GetLibs(con,db)
    PHASBatch(con,libs,geno,geno_index)
