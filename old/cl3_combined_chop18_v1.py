...
trascriptome_file = 'RICE_cdna_MSU7.fa'##The FASTA header will be cleaned

##miRNA File
miRNA_file = 'Rice_mirBASE_all.fa'##The header will be cleaned
...
def ConnectToDB():
    ...
    try:
        con=sql.connect(host='manyo.dbi.udel.edu', user='kakrana', passwd='livetheday')
...
def TagAbundanceFile(con):
    ...
    os.mkdir('./PARE')
    cur = con.cursor()
    cur.execute('select distinct(lib_id) from RICE_pub_PARE_v4.run_master')
    libs = cur.fetchall()
#    print (libs)
...
#        print (lib[0])
        ...
        cur.execute('select tag, raw_value from RICE_pub_PARE_v4.run_master where lib_id = %s', (lib[0],))
...
#        fh_in = openopen('%s_PARE_tags.fa' % (lib[0]), 'r')
        pipe = subprocess.Popen(["/data2/homes/kakrana/bowtie-0.12.7/bowtie", "-v", mismatch,"--best", "--strata", "-k", report_seq, "-f", "/data2/homes/kakrana/bowtie-0.12.7/indexes/rice_cdna_index", dd_file, map_out])
...
#    os.cd('./dd_map')
    ...
    os.mkdir('./dd_density')
    os.mkdir('./cl_results')
    seq_len = str(18)##For mapping PARE data on transcriptome
    pval = str(0.05)##For final cleveland command
...
##Summarize Degradome
        pipe = subprocess.Popen(["/data2/homes/kakrana/3-Cleaveland/CleaveLand3_map2dd.pl", "-d", map_file, "-t", filename, "-s", seq_len, "-f", "bowtie"], stdout=subprocess.PIPE, universal_newlines=True)
        ...
        pipe2 = subprocess.Popen(["/data2/homes/kakrana/3-Cleaveland/CleaveLand3_analysis.pl", "-d", density_file, "-t", "./target_finder_results/", "-p", pval], stdout=subprocess.PIPE, universal_newlines=True)
