...
def GetCoords(con,db):
    ...
    ###########################################################TEST LINE#################################################################
    #cur.execute("SELECT chr_id,strand,gene,start,end,type FROM %s.gene_master WHERE type NOT LIKE 'mirna' AND title NOT LIKE 'Ribosome RNA region approximations' AND chr_id = 1 ORDER BY chr_id,strand,start" % (db))###Limiting number of entries will effect last gene coords and sequence
    #####################################################################################################################################
...
def RevMapCoord(gene_coords, res_file, density_file): ####DOES THIS MODULE WORKS IF USER USES MULTIPLE LIBRARIES - Parallelize
    ...
    for res in fh_in: ## for every result reverse map co-ordinates to genomic and get PARE mappings in samll and large window
        ...
        #
        #geno_start = coord_dict_wat[gene_name]
        #geno_end = coord_dict_crick[gene_name]
        #print (geno_start)
        #print (cleave_site, bind_site)
        ...
        ####Get PARE abundace in small window (3 nt) and Large window (5nt) ONLY WORKS IF DB IS USED FOR PARE DATA, DOES NOT WORK IN CASE OF multiplel libraries
        ###Because in that case we need a lib id from which results were validated and than use that library to count PARE mappings in small and large window
        ...
        if found: ### gene_name from sco inp extend results found in PARE summarization file and his record is fetched
            ...
            for i in arecord[2:-1]:###In gene summarization record first two entries are gene_name and size of gene, last entry is a blank line when record is fetched (not in file)
                locus = i.split()
                #print(locus)
                #print (locus[0],cleave_site+1)
                if int(locus[0]) <= int(cleave_site+1) and int(locus[0]) >= int(cleave_site-1):####Small Window###Every 5' mapping on specific gene is read and if 5'end is mapped within small window than added
                    #print ('found %s' % locus[0])
                    small_win += (int(locus[1]))
                else: ###if the 5' end is not within the window - pass
                    pass
                
                if int(locus[0]) <= int(cleave_site+2) and int(locus[0]) >= int(cleave_site-2):###Large window###GET LARGE AND SMALL WINDOW IN SAME LOOP
