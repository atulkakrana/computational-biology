...
def TagAbundanceFile(con):
    ...
    os.mkdir('./PARE')
    cur = con.cursor()
    cur.execute('select distinct(lib_id) from RICE_priv_PARE_v4_MSU6.run_master')
    libs = cur.fetchall()
#    print (libs)
...
#        print (lib[0])
        ...
        cur.execute('select tag, raw_value from RICE_priv_PARE_v4_MSU6.run_master where lib_id = %s', (lib[0],))
