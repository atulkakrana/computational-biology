...
## Output file
fh_output=open('uniq_cleavage', 'w') 
...
fh_in.close()
fh_output.close()
###############Module 2- To identify Uniq patterns of Mir and their Targets i.e removing same mirna and same target

##Reading parsed output having redundant entries
fh_in=open('uniq_cleavage', 'r') 
#fh_in.readline()##Waste header line
parsed_in2=csv.reader(fh_in, delimiter=',')
#for i in parsed_in:
#    print(i)
...
added_keys2=set()## A set to store 2nd and 3rd elements from input file: mir and target and than use it to compare further entries in file

parsed_out_count=0## TO keep count of unique entries
for row in parsed_in2:
#    print(row)
    genename = row[0].split('.')[0]##To avoid different variations of same gene to be counted as uniq
    #print (genename)
    lookup=tuple((row[1],row[2]))##mir and target
    if lookup not in added_keys2:##That means such a tuple has not been recorded yet and is unique
        csv_out2.writerow([row[0]]+[row[1]]+[row[2]])##MiRNA name, miRNA seq and target seq
        parsed_out_count+=1
#        outlist.append(row)
        added_keys2.add(lookup)## Once a new entry is found it is recorded so as to compare and neglect further entries
    else:
        pass
