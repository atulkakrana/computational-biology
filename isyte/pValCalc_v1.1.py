...
testProbes  = 'AF_ALL_RESULTS_COMB_GLOB_v2.probe-Symb.txt' ## tab seprated file with probe ID as first column and gene symbol as second column

annoFile    = 'Mouse430_2.na34.annot.csv' ### Clean Header lines before using here
chip        = 1 ## Annotations for which chip - AF = 1 and IL =2
resFile     = 'AF_ALL_RESULTS_COMB_GLOB_v2.txt'
resSep      = '\t' ## Result file separator
keyCol      = 1 ## Key column in result file - AF = 1 | IL =2

rankStat    = 'FC' ## Possible options: 'FC' and 'TS'
cols        = [4,10] ## Affy Fold Change -  Column number and number of tracks (total 10 tracks in current file)
...
weight      = 7       ## Weight of Relative rank to use and popuRanks -Range 1 to 10
        ...
        if head[col].split('.')[0] == 'FC':  ## Confirms FC as column
            ...
            for ent in testProbeList: ##
                ...
                if weight < 10:
                    ...
                    mainRank = ( (mainIntRank*weight) + (mainPopuRank*(10-weight)) ) / 10
                    intRankArray = numpy.array(randomRanks)
                    popuRankArray = numpy.array(popuRanks)
                    randRanks = ( (intRankArray*weight)+(popuRankArray*(10-weight)) ) / 10
...
## Added 'bre' while calculating rabks in popuRankGenrator and relativeRankl generator to increae speed
